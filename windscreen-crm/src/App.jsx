...
const BUILD_NUMBER = "B102 · 18 Jul 2026";
...
function loadData() {
  ...
  return { customers: [], vehicles: [], jobs: [], invoices: [], mileage: [], inspections: [], communications: [], settings: [], timeOff: [], leads: [], technicians: [] };
}
...
async function emailStatementToCustomer(data, customer) {
  ...
  const res = await fetch("/api/send-statement", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ to: customer.email, customerName: customer.company || customer.companyContact || "", rows, totalOwed }),
  });
  const json = await res.json();
  if (!res.ok) throw new Error(json.error || "Send failed");
}
