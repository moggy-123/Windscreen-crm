...
const BUILD_NUMBER = "B77 · 18 Jul 2026";
...
function MileageView({ data, setView }) {
  ...
  async function runEstimate() {
    ...
    const lines = [];
    for (const j of jobsThatDay) {
      const customer = data.customers.find(c => c.id === j.customerId);
      const miles = await estimateRoundTripMiles(j.locPostcode);
      lines.push({ job: j, customer, miles });
    }
  ...
  async function addEstimateToLog() {
    ...
    const names = estimateResult.lines.map(l => l.customer?.company || l.customer?.companyContact || "Job").join(", ");
        ...
        {estimateResult && typeof estimateResult === "object" && (
          <>
            {estimateResult.lines.map((l, i) => (
              <div key={i} style={{ display:"flex", justifyContent:"space-between", fontSize:13, color:"#374151", padding:"4px 0" }}>
                <span>{l.customer?.company || l.customer?.companyContact || "Job"} — {l.job.locPostcode}</span>
