...
const BUILD_NUMBER = "B92 · 18 Jul 2026";
...
function openStatementWindow(data, customer) {
  ...
  const rows = unpaid.flatMap((inv) => {
    ...
    const overdueTag = overdue ? ' <span style="color:#DC2626;font-weight:700;">(overdue)</span>' : "";
    ...
    if (inv.lineItems?.length > 1) {
      ...
      <tr>
        ...
        <td style="padding:10px 12px;border-bottom:1px solid #E5E7EB;font-size:13px;color:#111827;">${desc || "—"}${idx === 0 ? overdueTag : ""}</td>
        <td style="padding:10px 12px;border-bottom:1px solid #E5E7EB;font-size:13px;color:#111827;text-align:right;font-weight:700;">£${(parseFloat(li.price)||0).toFixed(2)}</td>
      </tr>`;
      });
    }
    ...
    return [`
      <tr>
        <td style="padding:10px 12px;border-bottom:1px solid #E5E7EB;font-size:13px;color:#6B7280;">${fmtDate(dateStr)}</td>
        <td style="padding:10px 12px;border-bottom:1px solid #E5E7EB;font-size:13px;color:#111827;">${desc || "—"}${overdueTag}</td>
...
<html><head><script>try{window.opener=null;}catch(e){}</script><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Statement of Account</title>
<style>
...
function DamageReportModal({ customer, vehicles, data, onClose }) {
  ...
  function generate() {
    ...
    const html = `<!DOCTYPE html>
<html><head><script>try{window.opener=null;}catch(e){}</script><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Windscreen Damage Report</title>
<style>
  body { margin:0; padding:0; background:#F8FAFC; font-family:Arial,sans-serif; }
  @media print { .no-print { display:none !important; } body { background:#fff; } }
</style></head><body>
...
function CustomerDetail({ data, id, setView }) {
  ...
  const [termsSendStatus, setTermsSendStatus] = useState(null); // null | "sending" | "sent" | "error"
      ...
      <Card>
        ...
        <div style={{ display:"flex", gap:8, marginTop:12, flexWrap:"wrap" }}>
          ...
          {customer.email && customer.custType === "Trade" && (
            <Btn size="sm" variant="ghost" disabled={termsSendStatus==="sending"} onClick={async () => {
              setTermsSendStatus("sending");
              try {
                await sendTermsToCustomer(customer);
                quickLog("Email", "Terms & Conditions emailed", {}, true);
                setTermsSendStatus("sent");
              } catch (e) {
                setTermsSendStatus("error");
                alert("Couldn't send the terms: " + (e?.message || e));
              }
            }}>{termsSendStatus==="sending" ? "Sending…" : termsSendStatus==="sent" ? "✓ Sent" : "📜 Email T&Cs"}</Btn>
          )}
