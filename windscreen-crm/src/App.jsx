...
const BUILD_NUMBER = "B111 · 18 Jul 2026";
...
function JobDetail({ data, id, setView }) {
  ...
  function invoicePayload() {
    ...
    return {
      ...
      includeJobCard,
      jobCard: includeJobCard ? {
        vehicleText: vehicle ? [vehicle.make, vehicle.model, vehicle.reg].filter(Boolean).join(" · ") : "",
        driverName: job.driverName || "",
        date: fmtDate(job.date),
        damageLines: reps.map(r => describeRepair(r) || r.type).join(", "),
        notes: job.notes || "",
        photosBefore: (job.photosBefore || []).filter(p => p.url).map(p => ({ url: p.url })),
        photosAfter: (job.photosAfter || []).filter(p => p.url).map(p => ({ url: p.url })),
      } : null,
    };
  }
        ...
        <Card style={{ background:"#F0FDF4", borderColor:"#BBF7D0" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            ...
            <label style={{ display:"flex", alignItems:"center", gap:8, cursor:"pointer", fontSize:13, color:"#374151", margin:"0 0 10px" }}>
              ...
              Also include Job Card{(job.photosBefore?.length || job.photosAfter?.length) ? " (with photos)" : ""}
            </label>
