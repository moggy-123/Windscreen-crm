...
const BUILD_NUMBER = "B85 · 18 Jul 2026";
...
<html><head><script>try{window.opener=null;}catch(e){}</script><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Terms and Conditions</title>
<style>
...
// Pure, synchronous — opens a proper invoice document (as opposed to the Job Card,
// which is a job completion report). Handles both a simple single-job invoice and a
// Trade customer's combined invoice, showing every vehicle as its own line.
function openInvoiceWindow(data, invoice, customer) {
  const fmtD = new Date().toLocaleDateString("en-GB");
  const toEmail = customer?.email || "";
  const subject = encodeURIComponent(`Invoice — ${customer?.company || customer?.companyContact || ""}`);
  const bodyText = encodeURIComponent(`Please find our invoice attached, totalling £${parseFloat(invoice.total||0).toFixed(2)}.\n\nWindscreen Repairs (Bristol)\n07946 222246`);
  const mailtoLink = `mailto:${toEmail}?subject=${subject}&body=${bodyText}`;
  ...
  if (invoice.lineItems?.length) {
    rows = invoice.lineItems.map((li, i) => `
      <tr>
        ...
        <td style="padding:10px 12px;border-bottom:1px solid #E5E7EB;font-size:13px;color:#111827;">${(li.description||"").replace(/\n/g,"<br>")}</td>
    ...
    subtotal = invoice.lineItems.reduce((s,li) => s + (parseFloat(li.price)||0), 0) + (parseFloat(invoice.parts)||0);
  } else {
    rows = `
      <tr>
        <td style="padding:10px 12px;border-bottom:1px solid #E5E7EB;font-size:13px;color:#6B7280;">1</td>
        <td style="padding:10px 12px;border-bottom:1px solid #E5E7EB;font-size:13px;color:#111827;">${(invoice.details||"Windscreen Repair").replace(/\n/g,"<br>")}</td>
        <td style="padding:10px 12px;border-bottom:1px solid #E5E7EB;font-size:13px;color:#111827;text-align:right;font-weight:700;">£${(parseFloat(invoice.labour)||0).toFixed(2)}</td>
      </tr>`;
    subtotal = (parseFloat(invoice.labour)||0) + (parseFloat(invoice.parts)||0);
  }
  if (parseFloat(invoice.parts) > 0) {
    rows += `
      <tr>
        <td style="padding:10px 12px;border-bottom:1px solid #E5E7EB;font-size:13px;color:#6B7280;"></td>
        <td style="padding:10px 12px;border-bottom:1px solid #E5E7EB;font-size:13px;color:#111827;">Parts</td>
        <td style="padding:10px 12px;border-bottom:1px solid #E5E7EB;font-size:13px;color:#111827;text-align:right;font-weight:700;">£${parseFloat(invoice.parts).toFixed(2)}</td>
      </tr>`;
  }

  const html = `<!DOCTYPE html>
<html><head><script>try{window.opener=null;}catch(e){}</script><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Invoice</title>
<style>
  body { margin:0; padding:0; background:#F8FAFC; font-family:Arial,sans-serif; }
  @media print { .no-print { display:none !important; } body { background:#fff; } }
</style></head><body>
...
<div style="max-width:700px;margin:0 auto;padding:24px;background:#fff;">
  ...
  <div style="font-size:16px;font-weight:800;color:#1E3A5F;margin-bottom:4px;">Invoice</div>
  <div style="font-size:13px;color:#6B7280;margin-bottom:2px;">${customer?.company || customer?.companyContact || ""}</div>
  <div style="font-size:12px;color:#9CA3AF;margin-bottom:2px;">Invoice date: ${fmtD}</div>
  ${invoice.sageInvoiceNo ? `<div style="font-size:12px;color:#9CA3AF;margin-bottom:14px;">Reference: ${invoice.sageInvoiceNo}</div>` : '<div style="margin-bottom:14px;"></div>'}
  <table style="width:100%;border-collapse:collapse;border:1px solid #E5E7EB;">
    <thead><tr style="background:#F9FAFB;">
  ...
  <div style="margin-top:12px;">
    ${invoice.vat ? `
    <div style="display:flex;justify-content:space-between;font-size:13px;color:#6B7280;padding:4px 0;"><span>Subtotal</span><span>£${subtotal.toFixed(2)}</span></div>
    <div style="display:flex;justify-content:space-between;font-size:13px;color:#6B7280;padding:4px 0;"><span>VAT (20%)</span><span>£${(subtotal*0.2).toFixed(2)}</span></div>` : ""}
  </div>
  <div style="display:flex;justify-content:space-between;align-items:center;background:#F0FDF4;border:1px solid #BBF7D0;border-radius:8px;padding:14px 16px;margin-top:10px;">
    <span style="font-size:14px;font-weight:700;color:#065F46;">Total${invoice.vat ? " (inc. VAT)" : ""}</span>
    <span style="font-size:20px;font-weight:800;color:#065F46;">£${parseFloat(invoice.total||0).toFixed(2)}</span>
  </div>
  <div style="font-size:12px;color:${invoice.paid?"#059669":"#D97706"};font-weight:700;margin-top:10px;">${invoice.paid ? `✓ Paid${invoice.paidDate ? " " + fmtDate(invoice.paidDate) : ""}` : (customer?.custType === "Trade" ? "Payment due within 30 days" : "Payment due by return — please pay promptly using the details below")}</div>
  <div style="background:#F9FAFB;border:1px solid #E5E7EB;border-radius:8px;padding:14px 16px;margin-top:20px;">
    <div style="font-size:11px;font-weight:700;color:#6B7280;text-transform:uppercase;letter-spacing:.04em;margin-bottom:8px;">Payment Details</div>
    <div style="font-size:13px;color:#111827;line-height:1.6;">
      David Morgan trading as Windscreen Repairs (Bristol)<br>
      Account number: 02340725<br>
...
function JobDetail({ data, id, setView }) {
  ...
  // Downloads the exact PDF that would be sent, so it can be checked before it actually
  // goes out. Uses a direct file download (not a viewable tab) — deliberately, since a
  // download trigger can't leak the app's real address the way an opened tab could.
  async function previewInvoice() {
    ...
    try {
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `invoice-preview-${invoice.sageInvoiceNo || "draft"}.pdf`;
      a.click();
      setTimeout(() => URL.revokeObjectURL(url), 1000);
        ...
        <Card style={{ background:"#F0FDF4", borderColor:"#BBF7D0" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            ...
            <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
              ...
              <Btn size="sm" variant="ghost" onClick={() => openInvoiceWindow(data, invoice, customer)}>📄 View / Print</Btn>
