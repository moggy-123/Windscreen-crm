...
const BUILD_NUMBER = "B90 · 18 Jul 2026";
...
function openStatementWindow(data, customer) {
  ...
  const rows = unpaid.map((inv, i) => {
    ...
    if (inv.lineItems?.length > 1) {
      ...
      vehicleText = regs.join(", ");
    } else {
      vehicleText = inv.vehicle ? inv.vehicle.reg : "";
    }
    const desc = [inv.job?.jobType, vehicleText, inv.sageInvoiceNo ? `Inv ${inv.sageInvoiceNo}` : ""].filter(Boolean).join(" · ");
    return `
      <tr>
