...
const BUILD_NUMBER = "B105 · 18 Jul 2026";
...
async function sendTermsToCustomer(customer) {
...
async function emailStatementToCustomer(data, customer) {
  const { rows, totalOwed } = buildStatementRows(data, customer);
  const res = await fetch("/api/send-statement", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ to: customer.email, customerId: customer.id, customerName: customer.company || customer.companyContact || "", rows, totalOwed }),
  });
  const json = await res.json();
  if (!res.ok) throw new Error(json.error || "Send failed");
}
        ...
        <div style={{ display:"flex", gap:8, marginTop:12, flexWrap:"wrap" }}>
          ...
          {customer.custType === "Trade" && getUnpaidInvoices(data, customer.id).length > 0 && (
            <>
              ...
              {customer.email && (
                <Btn size="sm" variant="ghost" onClick={async () => { try { await emailStatementToCustomer(data, customer); alert("Statement sent."); } catch (e) { alert("Couldn't send the statement: " + (e?.message || e)); } }}>✉️ Statement</Btn>
              )}
        ...
        <Card style={{ background:"#F0FDF4", borderColor:"#BBF7D0" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            ...
            <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
              ...
              {customer?.email && (
                <Btn size="sm" onClick={emailInvoiceNow} disabled={emailStatus==="sending" || !invoice.sageInvoiceNo}>
