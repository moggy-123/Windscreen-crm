...
const BUILD_NUMBER = "B112 · 18 Sep 2026";
...
function Dashboard({ data, setView, notifStatus, requestNotifications }) {
  ...
  return (
    <div>
      ...
      <div style={{ marginTop:10 }}>
        <Btn onClick={() => setView({ screen:"photoShowcase" })} variant="ghost" style={{ width:"100%", justifyContent:"center" }}>📸 Photo Showcase</Btn>
      </div>
      <div style={{ marginTop:10 }}>
        <Btn onClick={openDemoDamageReport} variant="ghost" style={{ width:"100%", justifyContent:"center" }}>📋 Sample Damage Report</Btn>
      </div>
      <div style={{ marginTop:10 }}>
        <Btn onClick={() => setView({ screen:"leads" })} variant="ghost" style={{ width:"100%", justifyContent:"center" }}>🤝 New Business Leads</Btn>
      </div>
      <div style={{ marginTop:10 }}>
...
function ReportsView({ data }) {
  ...
  const rangeStats = (() => {
    let billedR = 0, receivedR = 0, jobsR = 0;
    (data.invoices || []).forEach(inv => {
      const amt = parseFloat(inv.total) || 0;
      const job = (data.jobs || []).find(j => j.id === inv.jobId);
      const billedDate = job?.date || inv.createdAt;
    ...
    (data.jobs || []).forEach(j => { if (j.date && j.date >= rangeFrom && j.date <= rangeTo) jobsR += 1; });
    return { billedR, receivedR, jobsR };
  })();
  ...
  return (
    <div>
      ...
      <div style={{ background:"#fff", border:"1px solid #F3F4F6", borderRadius:12, padding:16, marginBottom:20 }}>
        ...
        <p style={{ fontSize:11, color:"#9CA3AF", margin:"10px 0 0" }}>"Billed" is by job date (when the work was done); "Received" is by the date each invoice was actually marked paid.</p>
      </div>
