...
// Home base for mileage estimates — round trips are calculated from here and back.
const HOME_POSTCODE = "BS27 3FY";

// Looks up a UK location's coordinates via postcodes.io — free, no API key needed.
// Tries it as a postcode first; if that's not recognised, falls back to searching it
// as a place/area name instead (e.g. "Kingswood"), so an imprecise or missing postcode
// doesn't stop the whole estimate.
async function geocodePostcode(location) {
  ...
  // Fallback: search as a place name
  try {
    const res2 = await fetch(`https://api.postcodes.io/places?q=${encodeURIComponent(raw)}&limit=1`);
    const json2 = await res2.json();
    if (json2.status === 200 && json2.result?.length > 0) return { lat: json2.result[0].latitude, lng: json2.result[0].longitude };
  } catch {}
  return null;
}
...
// Estimates round-trip driving mileage from home to a postcode and back. Actual roads
// are never a straight line, so this applies a standard rule-of-thumb multiplier
// (~1.3x) to approximate real driving distance — a reasonable estimate, not a
// guarantee of the exact figure a mapping app would show.
async function estimateRoundTripMiles(postcode) {
  const home = await geocodePostcode(HOME_POSTCODE);
  const dest = await geocodePostcode(postcode);
  if (!home || !dest) return null;
  const straightLine = haversineMiles(home, dest);
  const roadEstimate = straightLine * 1.3;
  return Math.round(roadEstimate * 2); // round trip
}

// Estimates mileage for a whole day's route: Cheddar -> stop 1 -> stop 2 -> ... ->
// stop N -> Cheddar, in the order given — not a separate round trip per stop. Returns
// { legs: [{from, to, miles}], total } or null if any postcode couldn't be found.
async function estimateRouteMiles(postcodesInOrder) {
  const home = await geocodePostcode(HOME_POSTCODE);
  if (!home) return { legs: [], total: 0, failed: postcodesInOrder };

  const points = [];
  const failed = [];
  for (const pc of postcodesInOrder) {
    const coords = await geocodePostcode(pc);
    if (!coords) { failed.push(pc); continue; }
    points.push({ postcode: pc, coords });
  }
  if (points.length === 0) return { legs: [], total: 0, failed };

  const stops = [{ postcode: "Cheddar (home)", coords: home }, ...points, { postcode: "Cheddar (home)", coords: home }];
  const legs = [];
  let total = 0;
  for (let i = 0; i < stops.length - 1; i++) {
    const miles = Math.round(haversineMiles(stops[i].coords, stops[i+1].coords) * 1.3);
    legs.push({ from: stops[i].postcode, to: stops[i+1].postcode, miles });
    total += miles;
  }
...
const BUILD_NUMBER = "B125 · 26 Sep 2026";
...
const DAMAGE_TYPES    = ["Chip", "Crack", "Pit Fill", "Scratch"];
...
function fmtDate(iso) {
  if (!iso) return "";
  const datePart = iso.split("T")[0]; // strip any time/timezone suffix (e.g. from a timestamptz column)
  const [y, m, d] = datePart.split("-");
  return `${d}/${m}/${y}`;
}

// A reminder is overdue if its date is in the past, OR it's due today but its
// specific time (if one was set) has already passed. No time set = only goes
// overdue once the whole day has passed.
function isReminderOverdue(r) {
  if (!r || !r.dueDate) return false;
  const todayStr = todayISO();
  if (r.dueDate < todayStr) return true;
  if (r.dueDate > todayStr) return false;
  if (!r.dueTime) return false;
  const nowStr = new Date().toTimeString().slice(0, 5); // "HH:MM"
  return r.dueTime <= nowStr;
}
...
async function restoreBackup(file) {
  ...
  if (!navigator.onLine) {
    alert("You're offline — Restore needs a working connection throughout. Nothing has been changed. Please get back on wifi or signal and try again.");
    return;
  }
  ...
  try {
    ...
    const tables = [
      { app: "customers", db: "customers" },
      { app: "vehicles", db: "vehicles" },
      { app: "jobs", db: "jobs" },
      { app: "invoices", db: "invoices" },
      { app: "mileage", db: "mileage" },
      { app: "inspections", db: "inspections" },
      { app: "communications", db: "communications" },
    ...
    for (const t of tables) {
      const backupIds = new Set((backup[t.app]||[]).map(r => r.id));
      for (const rec of (cloud[t.app]||[])) {
        if (!backupIds.has(rec.id)) {
          try { await deleteRecord(t.db, rec.id); } catch { failCount++; failedTables.add(t.db); }
        }
      }
      for (const rec of (backup[t.app]||[])) {
        try { await pushOne(t.db, rec); } catch (e) { failCount++; failedTables.add(t.db); console.warn(`Restore: failed on ${t.db}`, rec.id, e); }
      }
    }
    ...
    hideSavingOverlay();
    if (failCount > 0) {
      alert(`Restore mostly finished, but ${failCount} record(s) didn't go through (in: ${[...failedTables].join(", ")}) — likely a connection hiccup partway through.\n\nThis is safe to fix: once you're back on a strong connection, just run Restore Backup again with this exact same file. It always matches the database to whatever's in the file, so running it again will pick up anything that was missed — nothing will be duplicated or made worse.`);
    } else {
...
function Input({ value, onChange, type="text", placeholder, required }) {
  return (
    <div>
      <input style={inputStyle} type={type} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} required={required} />
      {type === "date" && value && <div style={{ fontSize:12, color:"#6B7280", marginTop:3 }}>{fmtDate(value)}</div>}
    </div>
  );
}
...
function ReminderModal({ data, onClose, customerId, contactOptions, initialContactName = "", initialPhone = "", initialNote = "" }) {
  ...
  async function save() {
    ...
    setSaving(true);
    const rec = { id: uid(), customerId: customerId || null, contactName: contactName.trim(), phone: phone.trim(), note: note.trim(), dueDate, dueTime, done: false, createdAt: new Date().toISOString() };
    try {
  ...
  return (
    <Modal title="Set Reminder" onClose={onClose}>
      ...
      <div style={{ display:"flex", gap:10 }}>
        <div style={{ flex:1 }}><Field label="Call back on" required><Input type="date" value={dueDate} onChange={setDueDate} /></Field></div>
        <div style={{ flex:1 }}><Field label="Time (optional)"><Input type="time" value={dueTime} onChange={setDueTime} /></Field></div>
      </div>
...
function Dashboard({ data, setView, notifStatus, requestNotifications }) {
  ...
  // Trade customers with at least one invoice more than 30 days old, still unpaid
  const oneMonthAgo = (() => { const d = new Date(); d.setMonth(d.getMonth()-1); return d.toISOString().split("T")[0]; })();
  const overdueTradeCustomers = data.customers.filter(c => c.custType === "Trade").filter(c => {
    const theirJobIds = new Set(data.jobs.filter(j => j.customerId === c.id).map(j => j.id));
    return data.invoices.some(inv => !inv.paid && theirJobIds.has(inv.jobId) && (data.jobs.find(j=>j.id===inv.jobId)?.date || inv.createdAt || "") < oneMonthAgo);
  });
  ...
  return (
    <div>
      ...
      {overdueTradeCustomers.length > 0 && (
        <div onClick={() => setView({ screen:"settingsBalances" })} style={{ background:"#DC2626", color:"#fff", borderRadius:10, padding:"12px 14px", marginBottom:20, cursor:"pointer", display:"flex", alignItems:"center", gap:10 }}>
          <span style={{ fontSize:20 }}>⚠️</span>
          <div>
            <div style={{ fontSize:14, fontWeight:700 }}>{overdueTradeCustomers.length} Trade customer{overdueTradeCustomers.length===1?"":"s"} overdue (30+ days)</div>
            <div style={{ fontSize:12, opacity:0.9 }}>{overdueTradeCustomers.map(c => c.company || c.companyContact || "Unnamed").join(", ")} — tap to send statements</div>
          </div>
        </div>
      )}
      {(data.emailEvents || []).length > 0 && (
        <div onClick={() => setView({ screen:"emailIssues" })} style={{ background:"#DC2626", color:"#fff", borderRadius:10, padding:"12px 14px", marginBottom:20, cursor:"pointer", display:"flex", alignItems:"center", gap:10 }}>
          <span style={{ fontSize:20 }}>📧</span>
          <div>
            <div style={{ fontSize:14, fontWeight:700 }}>{data.emailEvents.length} Email Delivery Issue{data.emailEvents.length===1?"":"s"}</div>
            <div style={{ fontSize:12, opacity:0.9 }}>A bounce, spam complaint, or delay was reported — tap to see details</div>
          </div>
        </div>
      )}
      <div style={{ marginBottom:20 }}>
          ...
          <Card key={r.id}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", gap:8 }}>
              <div onClick={() => r.customerId && setView({ screen:"customerDetail", id:r.customerId })} style={{ cursor: r.customerId ? "pointer" : "default", flex:1 }}>
                ...
                <div style={{ fontSize:12, color: isReminderOverdue(r) ? "#DC2626" : "#D97706", fontWeight:600, marginTop:2 }}>
                  {isReminderOverdue(r) ? "⚠️ Overdue · " : "Due today · "}{fmtDate(r.dueDate)}{r.dueTime ? ` at ${r.dueTime}` : ""}
                </div>
              </div>
        ...
        <div style={{ background:"#F8FAFC", border:"1px solid #E5E7EB", borderRadius:10, padding:12, marginBottom:14 }}>
          ...
          {contacts.map((ct, idx) => (
            <div key={ct.id} style={{ background:"#fff", border: ct.main ? "1.5px solid #F59E0B" : "1px solid #F3F4F6", borderRadius:8, padding:10, marginBottom:8 }}>
              ...
              <div style={{ marginBottom:8 }}><Input value={ct.email} onChange={v => updateContact(ct.id, "email", v)} placeholder="Email" type="email" /></div>
              <div style={{ fontSize:11, fontWeight:700, color:"#9CA3AF", marginBottom:4, textTransform:"uppercase", letterSpacing:"0.05em" }}>Receives</div>
              <div style={{ display:"flex", flexWrap:"wrap", gap:10 }}>
                {[["receivesInvoices","Invoices"],["receivesStatements","Statements"],["receivesJobReports","Job Reports"],["receivesOffers","Offers"]].map(([field,label]) => (
                  <label key={field} style={{ display:"flex", alignItems:"center", gap:5, fontSize:12, color:"#374151", cursor:"pointer" }}>
                    <input type="checkbox" checked={!!ct[field]} onChange={e => updateContact(ct.id, field, e.target.checked)} style={{ width:14, height:14 }} />
                    {label}
                  </label>
                ))}
              </div>
...
const TERMS_INTRO = "Please read these Terms and Conditions carefully. By booking a repair service with Windscreen Repairs Bristol (\"the Company\", \"we\", \"us\") verbally, over the phone, via text, or in writing, you (\"the Customer\", \"you\") agree to be bound by these terms.";
...
// Emails the real Terms & Conditions PDF (built server-side) to one customer. Throws on
// failure, so callers can decide how to handle/report it.
async function sendTermsToCustomer(customer, message) {
...
// Opens the real Terms & Conditions PDF for viewing, without sending anything.
async function previewTermsPdf() {
  const res = await fetch("/api/send-terms", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ previewOnly: true, sections: TERMS_SECTIONS, intro: TERMS_INTRO }),
  });
  const json = await res.json();
...
// Finds who a document should actually go to: the first contact tagged for that type
// (e.g. "receivesInvoices"), falling back to the customer's own email if nobody's tagged
// — so customers without contact-level routing set up keep working exactly as before.
function getRecipientEmail(customer, field) {
  const tagged = (customer.contacts || []).find(ct => ct[field] && ct.email);
  return tagged ? tagged.email : (customer.email || "");
}
...
// Builds the row data for a customer's statement — one row per vehicle for a combined
// invoice, matching how the on-screen Outstanding/Overdue lists already work.
function buildStatementRows(data, customer) {
  const unpaid = getUnpaidInvoices(data, customer.id);
  const totalOwed = unpaid.reduce((s,inv) => s + (parseFloat(inv.total)||0), 0);
  const oneMonthAgo = (() => { const d = new Date(); d.setMonth(d.getMonth()-1); return d.toISOString().split("T")[0]; })();

  const rows = unpaid.flatMap((inv) => {
    ...
    if (inv.lineItems?.length > 1) {
      return inv.lineItems.map((li, idx) => {
        ...
        return { date: fmtDate(dateStr), description: desc || "—", amount: li.price, overdue: idx === 0 && overdue };
      });
    }
    const desc = [inv.job?.jobType, inv.vehicle ? inv.vehicle.reg : "", inv.sageInvoiceNo ? `Inv ${inv.sageInvoiceNo}` : ""].filter(Boolean).join(" · ");
    return [{ date: fmtDate(dateStr), description: desc || "—", amount: inv.total, overdue }];
  });

  return { rows, totalOwed };
}

async function emailStatementToCustomer(data, customer, message) {
  const { rows, totalOwed } = buildStatementRows(data, customer);
  const res = await fetch("/api/send-statement", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ to: getRecipientEmail(customer, "receivesStatements"), customerId: customer.id, customerName: customer.company || customer.companyContact || "", rows, totalOwed, message }),
  });
  const json = await res.json();
  if (!res.ok) throw new Error(json.error || "Send failed");
}

async function previewStatementPdf(data, customer) {
  const { rows, totalOwed } = buildStatementRows(data, customer);
  const res = await fetch("/api/send-statement", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ previewOnly: true, customerName: customer.company || customer.companyContact || "", rows, totalOwed }),
  });
  const json = await res.json();
  if (!res.ok) throw new Error(json.error || "Preview failed");
  const byteChars = atob(json.pdfBase64);
...
// (Invoice, Terms, Statement); bulk sends stay instant by design, since a per-recipient
// review step doesn't scale to a chase-everyone-quickly workflow.
function ComposeEmailModal({ to, subject, defaultBody, sending, onSend, onClose }) {
  const [body, setBody] = useState(defaultBody);
  return (
    <Modal title="Review Email" onClose={onClose}>
      <Field label="To"><div style={{ fontSize:14, color:"#111827", padding:"8px 0" }}>{to}</div></Field>
      <Field label="Subject"><div style={{ fontSize:14, color:"#111827", padding:"8px 0" }}>{subject}</div></Field>
      <Field label="Message">
      ...
      <p style={{ fontSize:11, color:"#9CA3AF", margin:"0 0 14px" }}>The attached PDF isn't affected by this — only the email text above.</p>
      <Btn onClick={() => onSend(body)} disabled={sending} style={{ width:"100%", justifyContent:"center" }}>
  ...
  return (
    <div>
      ...
      {customerReminders.map(r => {
        ...
        return (
        <div key={r.id} style={{ background: overdue ? "#FEF2F2" : "#FFF7ED", border: overdue ? "1px solid #FECACA" : "1px solid #FED7AA", borderRadius:10, padding:"12px 16px", marginBottom:14, fontSize:14, display:"flex", justifyContent:"space-between", alignItems:"center", gap:10 }}>
          <div>
            <span style={{ fontWeight:700, color: overdue ? "#DC2626" : "#92400E" }}>{overdue ? "⚠️ " : ""}📞 Call {r.contactName} {fmtDate(r.dueDate)}{r.dueTime ? ` at ${r.dueTime}` : ""}</span>
            {r.note && <span style={{ color: overdue ? "#B91C1C" : "#B45309" }}> — {r.note}</span>}
          </div>
      ...
      <Card>
        ...
        {/* Primary actions — the ones used on nearly every visit stay right here;
            everything else is one tap away under "More actions" below, so the
            card doesn't turn into a wall of buttons. */}
        <div style={{ display:"flex", gap:8, marginTop:12, flexWrap:"wrap" }}>
          {customer.phone && (
            <a href={`tel:${customer.phone}`} style={{ textDecoration:"none", flex:1, minWidth:120 }} onClick={() => quickLog("Call", "")}>
              <Btn size="sm" variant="primary" style={{ width:"100%", justifyContent:"center" }}>📞 Call</Btn>
            </a>
          )}
          <Btn size="sm" variant="ghost" style={{ flex:1, minWidth:120, justifyContent:"center" }} onClick={() => { setEditingComm(null); setLogContact(null); setShowCommLog(true); }}>💬 Log / Message</Btn>
        </div>
        {customer.custType === "Trade" && getUnpaidInvoices(data, customer.id).length > 0 && (
          <div style={{ marginTop:8 }}>
            <Btn size="sm" style={{ width:"100%", justifyContent:"center" }} onClick={() => setShowRecordPayment(true)}>💷 Record Payment</Btn>
          </div>
        )}

        <button onClick={() => setShowMoreActions(v => !v)} style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:6, width:"100%", marginTop:10, padding:"10px 8px", borderRadius:8, border:"1.5px dashed #D1D5DB", background:"#F9FAFB", color:"#6B7280", fontSize:13, fontWeight:700, fontFamily:"inherit", cursor:"pointer" }}>
        ...
        {showMoreActions && (
          <div style={{ display:"flex", gap:8, marginTop:10, flexWrap:"wrap" }}>
            ...
            {customer.email && customer.custType === "Trade" && (
              <Btn size="sm" variant="ghost" onClick={() => setShowComposeTerms(true)}>📜 Email T&Cs</Btn>
            )}
            {data.jobs.some(j => j.customerId === customer.id) && <Btn size="sm" variant="ghost" onClick={() => setShowDaySheet(true)}>📋 Day Sheet</Btn>}
            {customer.custType === "Trade" && getUnpaidInvoices(data, customer.id).length > 0 && (
              <>
                <Btn size="sm" variant="ghost" onClick={async () => { try { await previewStatementPdf(data, customer); } catch (e) { alert("Couldn't build the preview: " + (e?.message || e)); } }}>👁️ Preview Statement</Btn>
                {getRecipientEmail(customer, "receivesStatements") && (
                  <Btn size="sm" variant="ghost" onClick={() => setShowComposeStatement(true)}>✉️ Statement</Btn>
                )}
              </>
            )}
      ...
      {showComposeStatement && (
        <ComposeEmailModal
          ...
          subject={`Statement of Account — ${customer.company || customer.companyContact || "Windscreen Repairs Bristol"}`}
          defaultBody={`Please find our statement attached, showing a total outstanding balance of £${buildStatementRows(data, customer).totalOwed.toFixed(2)}.\n\nWindscreen Repairs (Bristol)\n07946 222246`}
          onSend={async (message) => {
            try { await emailStatementToCustomer(data, customer, message); setShowComposeStatement(false); alert("Statement sent."); }
            catch (e) { alert("Couldn't send the statement: " + (e?.message || e)); }
          }}
          onClose={() => setShowComposeStatement(false)}
        />
      )}
      {showComposeTerms && (
        <ComposeEmailModal
          ...
          defaultBody={`Please find our current Terms and Conditions attached.\n\nWindscreen Repairs (Bristol)\n07946 222246`}
          onSend={async (message) => {
            try {
              await sendTermsToCustomer(customer, message);
              quickLog("Email", "Terms & Conditions emailed", {}, true);
              setShowComposeTerms(false);
            } catch (e) { alert("Couldn't send the terms: " + (e?.message || e)); }
          }}
          onClose={() => setShowComposeTerms(false)}
        />
      )}
          ...
          <Card key={j.id} onClick={() => setView({ screen:"jobDetail", id:j.id })}>
            <div style={{ display:"flex", justifyContent:"space-between" }}>
              ...
              <div style={{ display:"flex", gap:6, alignItems:"center" }}>
                {j.noCharge && <span style={{ fontSize:10, fontWeight:700, color:"#92400E", background:"#FFFBEB", padding:"2px 6px", borderRadius:5 }}>NO CHARGE</span>}
        ...
        {repairs.map((r, idx) => (
          <div key={r.id} style={{ background:"#F8FAFC", border:"1px solid #F3F4F6", borderRadius:10, padding:12, marginBottom:8 }}>
            ...
            <div style={{ display:"flex", gap:8 }}>
              ...
              <div style={{ flex:1 }}>
                <Select value={r.position} onChange={v => updateRepair(r.id, "position", v)} options={["Top","Top Right","Top Left","Top Centre","Centre","Centre Left","Centre Right","Bottom","Bottom Right","Bottom Left","Bottom Centre"]} placeholder="Position…" />
              </div>
            </div>
            ...
            {r.unrepairable && (
              <div style={{ marginTop:8 }}>
                <Select value={r.unrepairableReason||""} onChange={v => updateRepair(r.id, "unrepairableReason", v)} options={["Too large — Zone A","Too large — Zone B","In driver's vision (Zone A)","In ADAS camera zone","Edge crack / spreading"]} placeholder="Reason…" />
              </div>
            )}
...
<div class="no-print" style="position:sticky;top:0;z-index:100;background:#1E3A5F;padding:12px 16px;display:flex;gap:10px;align-items:center;justify-content:center;flex-wrap:wrap;">
  ...
  <div style="font-size:11px;color:#FCA5A5;font-weight:700;width:100%;text-align:center;margin-top:2px;">⚠️ On a computer: in the print dialog, turn OFF "Headers and Footers" first, or your private app address gets printed at the bottom</div>
  ...
  <button onclick="window.close()" style="background:rgba(255,255,255,.15);color:#fff;border:1.5px solid rgba(255,255,255,.4);border-radius:8px;padding:10px 20px;font-size:14px;font-weight:700;cursor:pointer;">✕ Close</button>
...
<div style="max-width:760px;margin:0 auto;padding:24px;background:#fff;">
  ...
  <div style="border-top:1px solid #E5E7EB;padding-top:16px;">
    ...
    // to a blob: URL — a blob URL's address bar reveals the real app domain (which a
    // browser's print footer can then expose); about:blank never does.
    const win = window.open("about:blank", "_blank");
    if (win) { win.document.open(); win.document.write(html); win.document.close(); }
    else { alert("Please allow pop-ups for this site, then try again."); }
  }
    ...
    <div>
      ...
      <Card>
        ...
        {(inspection.vehicles||[]).length > 0 && (() => {
          ...
          const repairedCount = vehicles.filter(v => v.bookedJobId && ["Complete","Invoiced","Paid"].includes(data.jobs.find(j => j.id === v.bookedJobId)?.status)).length;
          return (
            <div style={{ fontSize:13, fontWeight:600, marginTop:8, color: repairedCount===vehicles.length ? "#059669" : bookedCount>0 ? "#D97706" : "#9CA3AF" }}>
              {repairedCount===vehicles.length ? "✅ All vehicles repaired" : `${repairedCount} repaired · ${bookedCount-repairedCount} booked, not yet done · ${vehicles.length-bookedCount} not yet booked`}
            </div>
          );
        })()}
        <div style={{ display:"flex", gap:8, marginTop:12, flexWrap:"wrap" }}>
      ...
      {(inspection.vehicles||[]).map(v => {
        const linkedJob = v.bookedJobId ? data.jobs.find(j => j.id === v.bookedJobId) : null;
        return (
        <Card key={v.id}>
          <div onClick={() => { setEditingVehicle(v); setShowVehicleForm(true); }} style={{ cursor:"pointer" }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
              ...
              {linkedJob ? <StatusBadge status={linkedJob.status} /> : v.bookedJobId ? <span style={{ fontSize:10, fontWeight:700, color:"#9CA3AF", background:"#F3F4F6", padding:"3px 8px", borderRadius:6 }}>JOB DELETED</span> : <span style={{ fontSize:10, fontWeight:700, color:"#9CA3AF", background:"#F3F4F6", padding:"3px 8px", borderRadius:6 }}>NOT BOOKED</span>}
            </div>
            <div style={{ display:"flex", gap:6, flexWrap:"wrap", marginTop:8 }}>
              {(v.repairs||[]).map(r => (
                <span key={r.id} style={{ fontSize:11, fontWeight:600, color:"#92400E", background:"#FEF3C7", padding:"3px 8px", borderRadius:6 }}>{describeRepair(r) || r.type}</span>
          ...
          <div style={{ display:"flex", gap:8, marginTop:8 }}>
            {linkedJob && <Btn size="sm" variant="ghost" onClick={() => setView({ screen:"jobDetail", id:linkedJob.id })}>View Job</Btn>}
            <Btn size="sm" variant="danger" onClick={() => removeVehicle(v.id)}><Icon name="trash" size={12} /> Remove</Btn>
          </div>
        </Card>
        );
...
function JobsList({ data, setView, initialFilter }) {
  ...
  const filtered = data.jobs.filter(j => {
    ...
    // Open/Today are "what's coming up" lists — soonest first. Unpaid/To Invoice/All
    // read more like a history log, so those stay most-recent-first as before.
    const soonestFirst = filter === "Open" || filter === "Today";
    const dateCompare = soonestFirst ? a.date.localeCompare(b.date) : b.date.localeCompare(a.date);
    if (dateCompare !== 0) return dateCompare;
    // Same date — earliest job time first; jobs with no time set sink to the bottom
    if (!a.jobTime && !b.jobTime) return 0;
    if (!a.jobTime) return 1;
    if (!b.jobTime) return -1;
    return a.jobTime.localeCompare(b.jobTime);
  ...
  return (
    <div>
      ...
      {filtered.map(job => {
        const cust = data.customers.find(c => c.id === job.customerId);
        const veh  = data.vehicles.find(v => v.id === job.vehicleId);
        const repCount = job.repairs?.length ? job.repairs.length : (job.damageType ? 1 : 0);
        return (
          <Card key={job.id} onClick={() => setView({ screen:"jobDetail", id:job.id })}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
              <div style={{ flex:1, minWidth:0 }}>
                ...
                <div style={{ fontSize:13, color:"#6B7280", marginTop:2 }}>{veh ? `${veh.make} ${veh.model} · ${veh.reg}` : "No vehicle"}{repCount > 0 ? ` · ${repCount} repair${repCount === 1 ? "" : "s"}` : ""}</div>
              ...
              <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:4 }}>
                {job.noCharge && <span style={{ background:"#FFFBEB", color:"#92400E", border:"1px solid #FDE68A", fontSize:10, fontWeight:700, padding:"2px 7px", borderRadius:6, letterSpacing:"0.03em" }}>NO CHARGE</span>}
                <StatusBadge status={job.status} />
              </div>
            </div>
          </Card>
...
function JobForm({ data, onClose, editJob, prefill }) {
  ...
  const [driverPhone,   setDriverPhone]   = useState(editJob?.driverPhone   || "");
  ...
  const [noChargeReason, setNoChargeReason] = useState(editJob?.noChargeReason || "Unrepairable");
  ...
  async function save() {
    ...
    const rec = { customerId, driverName, driverPhone, contactName, vehicleId, date, jobTime, locAddress1, locAddress2, locTown, locCounty, locPostcode, jobType, repairs, damageType: first.type || "", damageSide: first.side || "", damagePosition: first.position || "", adasRequired, status, technicianId, notes, paymentType, noCharge, noChargeReason: noCharge ? noChargeReason : "", insuranceCo, claimNo, photosBefore, photosAfter };
    if (editJob) {
  ...
  return (
    ...
    <Modal title={editJob ? "Edit Job" : "New Job"} onClose={onClose}>
          ...
          <>
            {contacts.length > 0 && (
              <Field label="Company Contact (optional)">
                <select style={{ ...inputStyle, appearance:"none" }} value={contactName} onChange={e => {
                  setContactName(e.target.value);
                  // Convenience only — pre-fills the phone from a saved contact so it doesn't
                  // need typing again, but never overwrites something already typed in.
                  const ct = contacts.find(c => c.name === e.target.value);
                  if (ct?.phone && !driverPhone) setDriverPhone(ct.phone);
                }}>
            ...
            <Field label="Driver / Site Contact Phone (optional)"><Input value={driverPhone} onChange={setDriverPhone} placeholder="07… — for calling/texting them directly on the day" /></Field>
          </>
        );
      })()}
      {customerId && (
      ...
      <label style={{ display:"flex", alignItems:"center", gap:8, marginBottom: noCharge ? 8 : 16, cursor:"pointer", background: noCharge ? "#FFFBEB" : "#F9FAFB", border: noCharge ? "1.5px solid #FDE68A" : "1.5px solid #E5E7EB", borderRadius:8, padding:"10px 12px" }}>
        <input type="checkbox" checked={noCharge} onChange={e => setNoCharge(e.target.checked)} style={{ width:18, height:18 }} />
        <span style={{ fontSize:14, color:"#92400E", fontWeight:600 }}>No Charge</span>
      </label>
      {noCharge && (
        <div style={{ marginBottom:16 }}>
          <Field label="Reason"><Select value={noChargeReason} onChange={setNoChargeReason} options={["Unrepairable","Goodwill","Warranty","Other"]} /></Field>
        </div>
      )}
...
function sendJobCard(job, customer, vehicle, invoice) {
  ...
  const html = `<!DOCTYPE html>
<html><head><script>try{window.opener=null;}catch(e){}</script>
<meta charset="UTF-8">
...
<div class="card" style="max-width:600px;margin:16px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.08);">
  ...
  <div style="padding:24px 28px;">
    ...
    <div style="margin-top:28px;padding-top:16px;border-top:1px solid #F3F4F6;text-align:center;font-size:12px;color:#9CA3AF;">
      Thank you for choosing Windscreen Repairs (Bristol)
    </div>
  </div>
</div>
...
function openDaySheet(data, customer, dateISO) {
  ...
  const sections = jobs.map((job, i) => {
    ...
    <div style="border:1px solid #E5E7EB;border-radius:10px;padding:16px 18px;margin-bottom:14px;${i>0 ? 'page-break-inside:avoid;' : ''}">
      ...
      <table style="width:100%;border-collapse:collapse;">
        ...
        ${!isTrade ? row("Payment", job.noCharge ? `No Charge${job.noChargeReason ? " — " + job.noChargeReason : ""}` : job.paymentType) : ""}
        ${row("Notes", job.notes)}
      </table>
  ...
  const html = `<!DOCTYPE html>
<html><head><script>try{window.opener=null;}catch(e){}</script><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Job Sheet</title>
<style>
  body { margin:0; padding:0; background:#F8FAFC; font-family:Arial,sans-serif; }
  @media print { .no-print { display:none !important; } body { background:#fff; } }
</style></head><body>
...
<div style="max-width:700px;margin:0 auto;padding:24px;background:#fff;">
  <div style="display:flex;align-items:center;gap:14px;border-bottom:3px solid #F59E0B;padding-bottom:14px;margin-bottom:18px;">
    <img src="${LOGO_DATA_URI}" style="width:56px;height:56px;object-fit:contain;" />
    <div>
      <div style="font-size:20px;font-weight:800;color:#1E3A5F;">Windscreen Repairs (Bristol)</div>
      <div style="font-size:12px;color:#6B7280;">3 Goosander Grove, Cheddar, BS27 3FY · 07946 222246</div>
    </div>
...
// Lets a single payment be applied across several of a customer's separate unpaid
// invoices at once — e.g. one bank transfer that clears three different invoices.
function RecordPaymentModal({ customer, data, onClose }) {
  const unpaid = getUnpaidInvoices(data, customer.id);
  const [selected, setSelected] = useState(() => {
  ...
  function describeInvoice(inv) {
    if (inv.lineItems?.length > 1) {
      const regs = inv.lineItems.map(li => {
        const j2 = data.jobs.find(x => x.id === li.jobId);
        const v2 = j2?.vehicleId ? data.vehicles.find(v => v.id === j2.vehicleId) : null;
        return v2?.reg;
      }).filter(Boolean);
      return regs.join(", ") || "Combined invoice";
    }
    return [inv.job?.jobType, inv.vehicle?.reg].filter(Boolean).join(" · ") || "—";
  }
  ...
  return (
    <Modal title="Record Payment" onClose={onClose}>
      <p style={{ fontSize:13, color:"#6B7280", margin:"0 0 14px" }}>Tick whichever invoices this payment covers — untick any that aren't included.</p>
      {unpaid.length === 0 && <p style={{ fontSize:13, color:"#9CA3AF" }}>Nothing outstanding for this customer</p>}
      {unpaid.map(inv => (
        <label key={inv.id} style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 12px", border:"1px solid #F3F4F6", borderRadius:8, marginBottom:6, cursor:"pointer", background: selected[inv.id] ? "#EFF6FF" : "#fff" }}>
          <input type="checkbox" checked={!!selected[inv.id]} onChange={() => toggle(inv.id)} style={{ width:18, height:18 }} />
          <div style={{ flex:1 }}>
            <div style={{ fontWeight:600, fontSize:14, color:"#111827" }}>{describeInvoice(inv)}</div>
            <div style={{ fontSize:12, color:"#9CA3AF" }}>{fmtDate(inv.job?.date || inv.createdAt)}{inv.sageInvoiceNo ? ` · Inv ${inv.sageInvoiceNo}` : ""}</div>
          </div>
          <div style={{ fontWeight:700, fontSize:14, color:"#111827" }}>£{(parseFloat(inv.total)||0).toFixed(2)}</div>
        </label>
      ))}
      {unpaid.length > 0 && (
        <>
          ...
          <Btn onClick={confirm} disabled={selectedInvoices.length===0} style={{ width:"100%", justifyContent:"center" }}>
...
function JobDetail({ data, id, setView }) {
  ...
  const [mileageEstimate, setMileageEstimate] = useState(null); // null | "loading" | number | "error"
  ...
  function invoicePayload() {
    // A combined Trade invoice covers more than one job (one line item per vehicle) —
    // when the Job Card is included, build one card per covered job, not just the job
    // this detail screen happens to be showing, so every vehicle's sheet gets attached.
    const coveredJobIds = jobIdsForInvoice(invoice);
    const coveredJobs = (coveredJobIds.length ? coveredJobIds.map(jid => data.jobs.find(j => j.id === jid)).filter(Boolean) : [job]);
    return {
      to: getRecipientEmail(customer, "receivesInvoices"),
      customerId: customer.id,
      customerName: customer.company || customer.companyContact || "",
      invoiceDate: fmtDate(invoice.createdAt || job.date),
      ...
      vat: !!invoice.vat,
      total: invoice.total,
      sageInvoiceNo: invoice.sageInvoiceNo || "",
      custType: customer.custType,
      paid: !!invoice.paid,
      paidDate: invoice.paidDate ? fmtDate(invoice.paidDate) : "",
      includeJobCard,
      jobCards: includeJobCard ? coveredJobs.map(j => {
        const jVehicle = data.vehicles.find(v => v.id === j.vehicleId);
        const jReps = j.repairs?.length ? j.repairs : (j.damageType ? [{ type: j.damageType, side: j.damageSide, position: j.damagePosition }] : []);
        return {
          vehicleText: jVehicle ? [jVehicle.make, jVehicle.model, jVehicle.reg].filter(Boolean).join(" · ") : "",
          driverName: j.driverName || "",
          date: fmtDate(j.date),
          repairCount: jReps.length,
          damageLines: jReps.map(r => describeRepair(r) || r.type).join(", "),
          notes: j.notes || "",
          photosBefore: (j.photosBefore || []).filter(p => p.url).map(p => ({ url: p.url })),
          photosAfter: (j.photosAfter || []).filter(p => p.url).map(p => ({ url: p.url })),
        };
      }) : null,
    };
  }

  // Opens the exact PDF that would be sent, so it can be checked before it actually goes
  // out. Opens it directly for viewing (Safari's own PDF viewer) rather than forcing a
  // download — forcing a download was causing a layout glitch in the installed app.
  async function previewInvoice() {
    if (!invoice) return;
    setEmailStatus("previewing");
    try {
      const res = await fetch("/api/send-invoice", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...invoicePayload(), previewOnly: true }),
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || "Preview failed");
      const byteChars = atob(json.pdfBase64);
      ...
      if (!win) alert("Please allow pop-ups for this site, then try again.");
      setTimeout(() => URL.revokeObjectURL(url), 60000);
      setEmailStatus("previewed");
    } catch (e) {
      setEmailStatus("error");
      alert("Couldn't build the preview: " + (e?.message || e));
    }
  }

  async function emailInvoiceNow(message) {
    if (!invoice || !customer || !getRecipientEmail(customer, "receivesInvoices")) return;
    setEmailStatus("sending");
    try {
      const res = await fetch("/api/send-invoice", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...invoicePayload(), message }),
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || "Send failed");
      setEmailStatus("sent");
      silentSave(current => ({
        communications: [...(current.communications || []), { id: uid(), customerId: customer.id, contactId: "", contactName: "", type: "Email", direction: "out", note: `Invoice emailed (£${invoice.total})`, timestamp: Date.now(), createdAt: todayISO() }],
      }));
    } catch (e) {
      setEmailStatus("error");
      alert("Couldn't send the invoice: " + (e?.message || e));
    }
  }
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
        ...
        <div style={{ display:"flex", gap:6, alignItems:"center" }}>
          {job.noCharge && <span style={{ fontSize:11, fontWeight:700, color:"#92400E", background:"#FFFBEB", border:"1px solid #FDE68A", padding:"3px 8px", borderRadius:6 }}>NO CHARGE{job.noChargeReason ? ` · ${job.noChargeReason}` : ""}</span>}
          <StatusBadge status={job.status} />
        </div>
      </div>
      <Card>
        ...
        <Row label="Driver Phone" value={job.driverPhone || null} />
      ...
      {job.driverPhone && (
        <div style={{ display:"flex", gap:8, marginBottom:14 }}>
          <a href={`tel:${job.driverPhone}`} style={{ flex:1, textDecoration:"none" }}>
            <Btn style={{ width:"100%", justifyContent:"center" }}>📞 Call {job.driverName || "Driver"}</Btn>
          </a>
          <a href={`sms:${job.driverPhone}`} style={{ flex:1, textDecoration:"none" }}>
            <Btn variant="ghost" style={{ width:"100%", justifyContent:"center" }}>💬 Message</Btn>
          </a>
        </div>
      )}
      {job.locPostcode && (
        <Card>
          {mileageEstimate === null && (
            <Btn variant="ghost" onClick={async () => {
              setMileageEstimate("loading");
              const miles = await estimateRoundTripMiles(job.locPostcode);
              setMileageEstimate(miles === null ? "error" : miles);
            }} style={{ width:"100%", justifyContent:"center" }}>📍 Estimate Mileage (Cheddar round trip)</Btn>
          )}
          {mileageEstimate === "loading" && <p style={{ fontSize:13, color:"#9CA3AF", textAlign:"center", margin:0 }}>Working it out…</p>}
          {mileageEstimate === "error" && <p style={{ fontSize:13, color:"#DC2626", textAlign:"center", margin:0 }}>Couldn't estimate that postcode — check it's a valid UK postcode, or add the mileage manually.</p>}
          {typeof mileageEstimate === "number" && (
            <>
              <div style={{ fontSize:13, color:"#6B7280", marginBottom:10, textAlign:"center" }}>Estimated round trip from Cheddar to {job.locPostcode} and back: <b style={{ color:"#1E3A5F" }}>{mileageEstimate} miles</b></div>
              <p style={{ fontSize:11, color:"#9CA3AF", margin:"0 0 10px", textAlign:"center" }}>Estimated from postcode data, not an exact route — a reasonable figure, not a guarantee.</p>
              <Btn onClick={async () => {
                const mileage = [...(data.mileage || []), { id: uid(), date: job.date, miles: mileageEstimate, note: `${customer?.company || "Job"} — ${job.locPostcode} (estimated)`, createdAt: todayISO() }];
                await saveAndReload({ ...data, mileage });
              }} style={{ width:"100%", justifyContent:"center" }}>Add {mileageEstimate} miles to Mileage Log</Btn>
            </>
          )}
        </Card>
      )}
      ...
      {job.noCharge && !invoice ? (
        <p style={{ fontSize:13, color:"#92400E", background:"#FFFBEB", border:"1px solid #FDE68A", borderRadius:8, padding:"10px 12px", marginBottom:10, textAlign:"center" }}>
          💷 Marked No Charge{job.noChargeReason ? ` — ${job.noChargeReason}` : ""} — no invoice needed for this job.
        </p>
      ) : (
          ...
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            ...
            {!invoice.sageInvoiceNo && (
              <p style={{ fontSize:12, color:"#B45309", background:"#FFFBEB", border:"1px solid #FDE68A", borderRadius:8, padding:"8px 10px", margin:"8px 0" }}>
                ⚠️ Add the Sage Invoice Number (Edit) before this can be emailed.
              </p>
            )}
            <label style={{ display:"flex", alignItems:"center", gap:8, cursor:"pointer", fontSize:13, color:"#374151", margin:"0 0 10px" }}>
            ...
            <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
              ...
              {customer && getRecipientEmail(customer, "receivesInvoices") && (
                <Btn size="sm" onClick={() => setShowComposeInvoice(true)} disabled={emailStatus==="sending" || !invoice.sageInvoiceNo}>
                  {emailStatus==="sending" ? "Sending…" : emailStatus==="sent" ? "✓ Sent" : "✉️ Email Invoice"}
                </Btn>
              )}
      ...
      <div style={{ display:"flex", gap:8, marginTop:8, flexWrap:"wrap" }}>
        {customer && (
          <Btn size="sm" variant="primary" style={{ width:"100%", justifyContent:"center" }} onClick={() => setView({ screen:"customerDetail", id:customer.id })}>
            👤 Go to Contact
          </Btn>
        )}
      </div>
      ...
      {showReminder && (
        <ReminderModal
          ...
          initialPhone={job.driverPhone || customer?.phone || ""}
          onClose={() => setShowReminder(false)}
        />
      )}
      ...
      {showComposeInvoice && invoice && customer && (
        <ComposeEmailModal
          to={getRecipientEmail(customer, "receivesInvoices")}
          subject={`Invoice — ${customer.company || customer.companyContact || "Windscreen Repairs Bristol"}`}
          defaultBody={`Please find your invoice${includeJobCard ? " and job card" : ""} attached, totalling £${parseFloat(invoice.total).toFixed(2)}.\n\nWindscreen Repairs (Bristol)\n07946 222246`}
          sending={emailStatus==="sending"}
          onSend={async (message) => { await emailInvoiceNow(message); setShowComposeInvoice(false); }}
          onClose={() => setShowComposeInvoice(false)}
        />
      )}
    </div>
  );
...
function InvoiceForm({ data, jobId, editInvoice, onClose }) {
  ...
  const autoDetails = (reps.length > 1 ? `${reps.length} repairs:\n` : "") + reps.map(r => `${r.type || "Repair"}${r.side ? " – " + r.side : ""}${r.position ? " " + r.position : ""}`).join("\n");
...
function TradeInvoiceForm({ data, job, customer, editInvoice, onClose }) {
  const buildLine = (j) => {
    ...
    const repCountText = reps.length > 1 ? `${reps.length} repairs: ` : "";
    const desc = [vehicle ? `${vehicle.make} ${vehicle.model} · ${vehicle.reg}` : "", repCountText + reps.map(r => `${r.type||"Repair"}${r.side?" – "+r.side:""}${r.position?" "+r.position:""}`).join(", ")].filter(Boolean).join(" — ");
  ...
  // customer + date (started from a sibling job) — offered as a choice, not forced.
  ...
  // null = not yet decided (only relevant when existingCombined was found); "combine" or "separate" once chosen.
  const [choice, setChoice] = useState(existingCombined ? null : "separate");
  const baseInvoice = editInvoice || (choice === "combine" ? existingCombined : null);
  ...
  // be pasted straight into Sage — a stop-gap until the two are properly linked. Not
  // shown anywhere on the printed/emailed invoice, just this on-screen editing view.
  async function copyLineItemsForSage() {
    const lines = lineItems.map(li => `${li.description}\t${li.price}`);
    if (parseFloat(parts) > 0) lines.push(`Parts\t${parts}`);
    const text = lines.join("\n");
    try {
      await navigator.clipboard.writeText(text);
    } catch {
      // Fallback for browsers/webviews without clipboard API access
  ...
  // The useState initialisers above only ran once, before "combine" was chosen (since
  // that choice happens after this component first mounts) — this catches the state up
  // to actually include the existing invoice's line items once that choice is made.
  useEffect(() => {
    if (choice === "combine" && existingCombined) {
      let items = existingCombined.lineItems ? [...existingCombined.lineItems] : [];
      if (job && !items.some(li => li.jobId === job.id)) items = [...items, buildLine(job)];
      setLineItems(items);
  ...
  // Every hook above this point must always run, on every render — so this early return
  // for the choice screen has to come after all of them, never in between.
  if (existingCombined && choice === null) {
    const vehicleCount = existingCombined.lineItems?.length || 0;
    return (
      <Modal title="Create Invoice" onClose={onClose}>
        <p style={{ fontSize:13, color:"#1D4ED8", background:"#EFF6FF", border:"1px solid #BFDBFE", borderRadius:8, padding:"10px 12px", marginBottom:16 }}>
          📋 {customer.company || customer.companyContact} already has an invoice started for {fmtDate(job.date)}, covering {vehicleCount} vehicle{vehicleCount===1?"":"s"} totalling £{parseFloat(existingCombined.total||0).toFixed(2)}.
        </p>
        <Btn onClick={() => setChoice("combine")} style={{ width:"100%", justifyContent:"center", marginBottom:8 }}>Add this vehicle to that invoice</Btn>
        <Btn variant="ghost" onClick={() => setChoice("separate")} style={{ width:"100%", justifyContent:"center" }}>Keep this as a separate invoice</Btn>
      </Modal>
    );
  }
  ...
  return (
    <Modal title={baseInvoice ? "Edit Invoice" : "Create Invoice"} onClose={onClose}>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", margin:"0 0 8px" }}>
        <div style={{ fontSize:12, fontWeight:700, color:"#6B7280", textTransform:"uppercase", letterSpacing:"0.05em" }}>Line Items</div>
        <button onClick={copyLineItemsForSage} style={{ background: copied ? "#DCFCE7" : "#F3F4F6", color: copied ? "#15803D" : "#374151", border:"none", borderRadius:6, padding:"4px 10px", fontSize:12, fontWeight:600, cursor:"pointer", fontFamily:"inherit" }}>
          {copied ? "✓ Copied" : "📋 Copy for Sage"}
        </button>
      </div>
      ...
      {otherSameDayJobs.length > 0 && (
        <div style={{ marginBottom:14 }}>
          ...
          {otherSameDayJobs.map(j2 => {
            const v2 = j2.vehicleId ? data.vehicles.find(v => v.id === j2.vehicleId) : null;
            const repCount = j2.repairs?.length ? j2.repairs.length : (j2.damageType ? 1 : 0);
            return (
              <button key={j2.id} onClick={() => addJob(j2.id)} style={{ display:"block", width:"100%", textAlign:"left", background:"#F3F4F6", border:"1px dashed #D1D5DB", borderRadius:8, padding:"8px 10px", fontSize:13, color:"#374151", cursor:"pointer", marginBottom:5, fontFamily:"inherit" }}>
                + Add {v2 ? `${v2.make} ${v2.model} · ${v2.reg}` : j2.jobType}{repCount > 0 ? ` (${repCount} repair${repCount === 1 ? "" : "s"})` : ""}
              </button>
            );
          })}
        </div>
...
function scheduleNotifications(data) {
  ...
  // Reminders due today with a specific time set — alert (with sound) at that time
  const todayReminders = (data.reminders || []).filter(r => !r.done && r.dueDate === today && r.dueTime);
  todayReminders.forEach(r => {
    ...
    if (msToAlert > 0) {
      window._crmTimers.push(setTimeout(() => {
        sendNotification(
          `📞 Reminder — Call ${r.contactName || "contact"}`,
          r.note || `Due now at ${r.dueTime}`
        );
      }, msToAlert));
    }
  });
...
function CalendarView({ data, setView, device }) {
  ...
  function Agenda() {
    // Ongoing/upcoming time off — shown on today's date if already underway, so it
    // doesn't disappear from view partway through a multi-day period.
    const timeOffByDate = {};
    (data.timeOff || []).forEach(t => {
      if (!t.startDate || !t.endDate || t.endDate < todayISO) return;
      const showOn = t.startDate < todayISO ? todayISO : t.startDate;
      (timeOffByDate[showOn] = timeOffByDate[showOn] || []).push(t);
    });

    const upcoming = [...new Set([...Object.keys(jobsByDate), ...Object.keys(timeOffByDate)])].filter(d => d >= todayISO).sort();
    if (upcoming.length === 0) return <p style={{ fontSize:13, color:"#9CA3AF", textAlign:"center", marginTop:30 }}>Nothing scheduled</p>;
    return (
      <div>
        {upcoming.map(date => (
          <div key={date} style={{ marginBottom:16 }}>
            ...
            {(timeOffByDate[date] || []).map(t => (
              <Card key={t.id} style={{ background:"#FFFBEB", borderColor:"#FDE68A" }}>
                <div style={{ fontWeight:600, fontSize:14, color:"#B45309" }}>
                  🏖️ {t.startDate < todayISO ? "Ongoing" : "Time off"}{t.startDate !== t.endDate ? ` until ${fmtDate(t.endDate)}` : ""}{t.startTime ? ` · ${t.startTime}–${t.endTime}` : ""}
                </div>
                {t.reason && <div style={{ fontSize:12, color:"#92400E" }}>{t.reason}</div>}
              </Card>
            ))}
...
// ── Reports ───────────────────────────────────────────────────────────────────
// ── Settings ─────────────────────────────────────────────────────────────────
// ── Settings (landing) — clean navigation buttons, like Home ────────────────
function SettingsView({ data, setView }) {
  const tradeCustomers = (data.customers || []).filter(c => c.custType === "Trade");
  const oneMonthAgo = (() => { const d = new Date(); d.setMonth(d.getMonth()-1); return d.toISOString().split("T")[0]; })();
  const overdueCount = tradeCustomers.filter(c => getUnpaidInvoices(data, c.id).some(inv => (inv.job?.date || inv.createdAt || "") < oneMonthAgo)).length;
  const outdatedTermsCount = tradeCustomers.filter(c => c.email && (!c.termsSentAt || c.termsSentVersion !== TERMS_VERSION)).length;
  ...
  return (
    <div>
      ...
      <h2 style={{ fontSize:18, fontWeight:800, color:"#1E3A5F", margin:"0 0 16px" }}>Settings</h2>

      <Btn onClick={() => setView({ screen:"settingsTerms" })} variant="ghost" style={{ width:"100%", justifyContent:"space-between", marginBottom:10 }}>
        <span>📜 Terms & Conditions</span>
        {outdatedTermsCount > 0 && <span style={{ fontSize:11, fontWeight:700, color:"#B45309", background:"#FFFBEB", padding:"3px 8px", borderRadius:6 }}>{outdatedTermsCount} to send</span>}
      </Btn>
      <Btn onClick={() => setView({ screen:"settingsOffers" })} variant="ghost" style={{ width:"100%", justifyContent:"center", marginBottom:10 }}>💷 Trade Offers</Btn>
      <Btn onClick={() => setView({ screen:"settingsBalances" })} variant="ghost" style={{ width:"100%", justifyContent:"space-between", marginBottom:10 }}>
        <span>⚠️ Outstanding & Overdue</span>
        {overdueCount > 0 && <span style={{ fontSize:11, fontWeight:700, color:"#DC2626", background:"#FEF2F2", padding:"3px 8px", borderRadius:6 }}>{overdueCount} overdue</span>}
      </Btn>
      <Btn onClick={() => setView({ screen:"settingsPricing" })} variant="ghost" style={{ width:"100%", justifyContent:"center", marginBottom:10 }}>💰 Pricing</Btn>
      <Btn onClick={() => setView({ screen:"settingsTimeOff" })} variant="ghost" style={{ width:"100%", justifyContent:"center", marginBottom:10 }}>🏖️ Time Off</Btn>
      <Btn onClick={() => setView({ screen:"emailIssues" })} variant="ghost" style={{ width:"100%", justifyContent:"center", marginBottom:10, gap:8 }}>
        <span>📧 Email Delivery Issues</span>
        {(data.emailEvents || []).length > 0 && <span style={{ fontSize:11, fontWeight:700, color:"#DC2626", background:"#FEF2F2", padding:"3px 8px", borderRadius:6 }}>{data.emailEvents.length}</span>}
      </Btn>
...
function SettingsTermsView({ data, setView }) {
  ...
  const [sendStatus, setSendStatus] = useState(null); // null | "sending" | { sentCount, failed: [{name, error}] }
  const [previewStatus, setPreviewStatus] = useState(null); // null | "loading"

  async function emailSelectedTrade() {
    ...
    for (const c of targets) {
      try {
        await sendTermsToCustomer(c);
        sentIds.push(c.id);
      } catch (e) {
        failed.push({ name: c.company || c.companyContact || "Unknown", error: e?.message || "Unknown error" });
      }
    }

    if (sentIds.length > 0) {
      const newEntries = sentIds.map(cid => ({ id: uid(), customerId: cid, contactId: "", contactName: "", type: "Email", direction: "out", note: `Terms & Conditions emailed (v${TERMS_VERSION})`, timestamp: Date.now(), createdAt: todayISO() }));
      await silentSave(current => ({
        customers: current.customers.map(c => sentIds.includes(c.id) ? { ...c, termsSentAt: Date.now(), termsSentVersion: TERMS_VERSION } : c),
        communications: [...(current.communications || []), ...newEntries],
      }));
    }
    setSendStatus({ sentCount: sentIds.length, failed });
  }

  async function handlePreview() {
    setPreviewStatus("loading");
    try {
      await previewTermsPdf();
    } catch (e) {
      alert("Couldn't build the preview: " + (e?.message || e));
    }
  ...
  return (
    <div>
      ...
      <h2 style={{ fontSize:18, fontWeight:800, color:"#1E3A5F", margin:"0 0 12px" }}>Terms & Conditions</h2>
      ...
      <div style={{ background:"#fff", border:"1px solid #F3F4F6", borderRadius:12, padding:16, marginBottom:16 }}>
        ...
        <Btn variant="ghost" onClick={handlePreview} disabled={previewStatus==="loading"} style={{ width:"100%", justifyContent:"center", marginBottom:12 }}>
          {previewStatus==="loading" ? "Building…" : "👁️ Preview PDF"}
        </Btn>
        ...
        {withEmail.length > 0 && (
          <>
            ...
            <Btn onClick={emailSelectedTrade} disabled={selectedCount===0 || sendStatus==="sending"} style={{ width:"100%", justifyContent:"center", marginTop:6 }}>
              {sendStatus==="sending" ? "Sending…" : `✉️ Email Selected (${selectedCount})`}
            </Btn>
            {sendStatus && sendStatus !== "sending" && (
              <div style={{ marginTop:8, fontSize:12, textAlign:"center" }}>
                {sendStatus.sentCount > 0 && <div style={{ color:"#059669", fontWeight:600 }}>✓ Sent to {sendStatus.sentCount} customer{sendStatus.sentCount===1?"":"s"}</div>}
                {sendStatus.failed.length > 0 && (
                  <div style={{ color:"#DC2626", marginTop:4 }}>
                    ⚠️ Failed for: {sendStatus.failed.map(f => f.name).join(", ")}
                  </div>
                )}
              </div>
            )}
...
// ── Settings → Trade Offers ───────────────────────────────────────────────────
function SettingsOffersView({ data, setView }) {
  const tradeCustomers = (data.customers || []).filter(c => c.custType === "Trade").sort((a,b) => (a.company||"").localeCompare(b.company||"", undefined, { sensitivity:"base" }));
  const withEmail = tradeCustomers.filter(c => c.email);
  const noEmailWithPhone = tradeCustomers.filter(c => !c.email && c.phone);
...
07946 222246`
  ...
  return (
    <div>
      ...
      <h2 style={{ fontSize:18, fontWeight:800, color:"#1E3A5F", margin:"0 0 12px" }}>Trade Offers</h2>

      <div style={{ background:"#fff", border:"1px solid #F3F4F6", borderRadius:12, padding:16, marginBottom:16 }}>
        <h3 style={{ margin:"0 0 4px", fontSize:14, fontWeight:700, color:"#374151" }}>£30 Flat Rate Offer</h3>
        <p style={{ margin:"0 0 12px", fontSize:13, color:"#6B7280" }}>A one-off promotional message to Trade customers about the new flat rate. This doesn't change any prices in the app — set those under Pricing once customers take you up on it.</p>
        <Field label="Message">
...
// ── Settings → Outstanding & Overdue ─────────────────────────────────────────
function SettingsBalancesView({ data, setView }) {
  const tradeCustomers = (data.customers || []).filter(c => c.custType === "Trade").sort((a,b) => (a.company||"").localeCompare(b.company||"", undefined, { sensitivity:"base" }));
  const oneMonthAgo = (() => { const d = new Date(); d.setMonth(d.getMonth()-1); return d.toISOString().split("T")[0]; })();
  ...
  return (
    <div>
      ...
      <h2 style={{ fontSize:18, fontWeight:800, color:"#1E3A5F", margin:"0 0 12px" }}>Outstanding & Overdue</h2>
        ...
        {(() => {
          ...
          return overdue.map(({ customer, unpaid, overdueInvoices, total }) => (
            <div key={customer.id} style={{ display:"flex", alignItems:"center", gap:8, padding:"9px 10px", border:"1px solid #FEE2E2", background:"#FEF2F2", borderRadius:8, marginBottom:5 }}>
              ...
              <button onClick={async () => { try { await emailStatementToCustomer(data, customer); alert("Statement sent."); } catch (e) { alert("Couldn't send the statement: " + (e?.message || e)); } }} style={{ background:"#DC2626", color:"#fff", border:"none", borderRadius:6, padding:"6px 10px", fontSize:12, fontWeight:700, cursor:"pointer" }}>✉️ Statement</button>
            </div>
          ));
        })()}
      </div>
        ...
        {(() => {
          ...
          return withBalances.map(({ customer, unpaid, total }) => (
            <div key={customer.id} style={{ display:"flex", alignItems:"center", gap:8, padding:"9px 10px", border:"1px solid #F3F4F6", borderRadius:8, marginBottom:5 }}>
              ...
              <span style={{ fontSize:14, fontWeight:800, color:"#92400E" }}>£{total.toFixed(2)}</span>
              <button onClick={async () => { try { await emailStatementToCustomer(data, customer); alert("Statement sent."); } catch (e) { alert("Couldn't send the statement: " + (e?.message || e)); } }} style={{ background:"#1E3A5F", color:"#fff", border:"none", borderRadius:6, padding:"6px 10px", fontSize:12, fontWeight:700, cursor:"pointer" }}>✉️ Statement</button>
            </div>
          ));
        })()}
      </div>
...
// ── Settings → Pricing ────────────────────────────────────────────────────────
function SettingsPricingView({ data, setView }) {
  ...
  return (
    <div>
      ...
      <h2 style={{ fontSize:18, fontWeight:800, color:"#1E3A5F", margin:"0 0 12px" }}>Pricing</h2>
...
const EMAIL_EVENT_LABELS = {
  "email.bounced": { label: "Bounced", color: "#DC2626", bg: "#FEF2F2", icon: "⚠️" },
  "email.complained": { label: "Marked as spam", color: "#B45309", bg: "#FFFBEB", icon: "🚩" },
  "email.delivery_delayed": { label: "Delayed", color: "#6B7280", bg: "#F3F4F6", icon: "⏳" },
};
const DOC_TYPE_LABELS = { invoice: "Invoice", terms: "Terms & Conditions", statement: "Statement" };

// ── Settings → Email Delivery Issues ───────────────────────────────────────────
// Populated only by the Resend webhook (server-side) when a send genuinely fails to
// reach someone — a real answer to "did that email actually arrive?" instead of hoping.
function EmailIssuesView({ data, setView }) {
  const events = [...(data.emailEvents || [])].sort((a,b) => (b.createdAt||"").localeCompare(a.createdAt||""));

  async function dismiss(id) {
    try { await deleteRecord("email_events", id); } catch (e) { alert("Couldn't dismiss: " + (e?.message || e)); return; }
    const emailEvents = (data.emailEvents || []).filter(e => e.id !== id);
    await saveAndReload({ ...data, emailEvents });
  }
  ...
  return (
    <div>
      ...
      <h2 style={{ fontSize:18, fontWeight:800, color:"#1E3A5F", margin:"0 0 4px" }}>Email Delivery Issues</h2>
      <p style={{ fontSize:13, color:"#6B7280", margin:"0 0 16px" }}>Shows up here automatically whenever an invoice, statement, or terms email genuinely fails to reach someone — a bounce, a spam complaint, or a delay. Nothing to do unless something appears.</p>

      {events.length === 0 && (
        <div style={{ textAlign:"center", padding:"40px 20px", color:"#9CA3AF" }}>
          <div style={{ fontSize:32, marginBottom:8 }}>✅</div>
          <p style={{ fontSize:14 }}>No delivery issues — everything's been arriving fine.</p>
        </div>
      )}

      {events.map(ev => {
        const style = EMAIL_EVENT_LABELS[ev.eventType] || { label: ev.eventType, color: "#6B7280", bg: "#F3F4F6", icon: "•" };
        const customer = ev.customerId ? data.customers.find(c => c.id === ev.customerId) : null;
        return (
          <Card key={ev.id} style={{ background: style.bg, borderColor: style.color + "33" }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:8 }}>
              <div>
                <div style={{ fontWeight:700, fontSize:14, color: style.color }}>{style.icon} {style.label}</div>
                <div style={{ fontSize:13, color:"#111827", marginTop:4 }}>
                  {DOC_TYPE_LABELS[ev.docType] || ev.docType || "Email"}
                  {customer ? ` to ${customer.company || customer.companyContact || "Unknown"}` : ""}
                </div>
                <div style={{ fontSize:12, color:"#6B7280", marginTop:2 }}>{ev.recipient}</div>
                {ev.bounceMessage && <div style={{ fontSize:12, color:"#6B7280", marginTop:6, fontStyle:"italic" }}>"{ev.bounceMessage}"</div>}
                <div style={{ fontSize:11, color:"#9CA3AF", marginTop:6 }}>{ev.createdAt ? new Date(ev.createdAt).toLocaleString("en-GB") : ""}</div>
              </div>
              <button onClick={() => dismiss(ev.id)} style={{ background:"#fff", color:"#374151", border:"1px solid #E5E7EB", borderRadius:6, padding:"6px 10px", fontSize:12, fontWeight:600, cursor:"pointer", whiteSpace:"nowrap" }}>Dismiss</button>
            </div>
            {customer && (
              <Btn size="sm" variant="ghost" onClick={() => setView({ screen:"customerDetail", id:customer.id })} style={{ marginTop:10 }}>View Customer</Btn>
            )}
          </Card>
        );
      })}
...
// ── Settings → Time Off ───────────────────────────────────────────────────────
function SettingsTimeOffView({ data, setView }) {
  const entries = [...(data.timeOff || [])].sort((a,b) => (a.startDate||"").localeCompare(b.startDate||""));
  ...
  async function remove(id) {
    ...
    try { await deleteRecord("time_off", id); } catch (e) { alert("Delete failed: " + (e?.message || JSON.stringify(e))); return; }
    addTombstone(id);
    const timeOff = (data.timeOff || []).filter(t => t.id !== id);
    await saveAndReload({ ...data, timeOff });
  }
  ...
  return (
    <div>
      ...
      <div style={{ marginTop:16 }}>
        {entries.length === 0 && <p style={{ fontSize:13, color:"#9CA3AF" }}>No time off booked in yet</p>}
        {entries.map(t => (
          <Card key={t.id} style={t.id===editingId ? { background:"#EFF6FF", borderColor:"#BFDBFE" } : undefined}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", gap:8 }}>
              ...
              <div style={{ display:"flex", gap:6 }}>
                <button onClick={() => startEdit(t)} style={{ background:"#EFF6FF", color:"#1E3A5F", border:"none", borderRadius:6, padding:"6px 12px", fontSize:13, fontWeight:600, cursor:"pointer" }}>Edit</button>
                <button onClick={() => remove(t.id)} style={{ background:"#FEE2E2", color:"#DC2626", border:"none", borderRadius:6, padding:"6px 12px", fontSize:13, fontWeight:600, cursor:"pointer" }}>Delete</button>
              </div>
            </div>
          </Card>
        ))}
...
const LEAD_OUTCOMES = ["Interested", "Follow Up", "Not Interested", "Converted"];
const LEAD_OUTCOME_COLORS = {
  "Interested": { color:"#1D4ED8", bg:"#EFF6FF" },
  "Follow Up": { color:"#B45309", bg:"#FFFBEB" },
  "Not Interested": { color:"#6B7280", bg:"#F3F4F6" },
  "Converted": { color:"#059669", bg:"#ECFDF5" },
};

// ── New Business Leads — log prospecting visits, track outcomes, convert to a real
// Trade customer once they agree to use the service ─────────────────────────────
function LeadsView({ data, setView }) {
  const leads = [...(data.leads || [])].sort((a,b) => (b.visitDate||"").localeCompare(a.visitDate||""));
  const [showForm, setShowForm] = useState(false);
  const [editingLead, setEditingLead] = useState(null);
  const [filter, setFilter] = useState("all");

  const total = leads.length;
  const converted = leads.filter(l => l.outcome === "Converted").length;
  const rate = total > 0 ? Math.round((converted / total) * 100) : 0;

  const filtered = filter === "all" ? leads : leads.filter(l => l.outcome === filter);

  async function saveLead(lead) {
    const existing = data.leads || [];
    const leads = existing.some(l => l.id === lead.id) ? existing.map(l => l.id === lead.id ? lead : l) : [...existing, lead];
    setShowForm(false); setEditingLead(null);
    await saveAndReload({ ...data, leads });
  }
  async function deleteLead(id) {
    if (!window.confirm("Delete this lead? This cannot be undone.")) return;
    try { await deleteRecord("leads", id); } catch (e) { alert("Delete failed: " + (e?.message || JSON.stringify(e))); return; }
    addTombstone(id);
    const leads = (data.leads || []).filter(l => l.id !== id);
    await saveAndReload({ ...data, leads });
  }
  async function convertToCustomer(lead) {
    if (!window.confirm(`Add "${lead.businessName || lead.contactName}" as a Trade customer?`)) return;
    const custId = uid();
    const customers = [...data.customers, {
      id: custId,
      company: lead.businessName || "",
      companyContact: lead.contactName || "",
      phone: lead.phone || "",
      email: lead.email || "",
    ...
    const leads = (data.leads || []).map(l => l.id === lead.id ? { ...l, outcome: "Converted", convertedCustomerId: custId } : l);
    try {
      await saveAndReload({ ...data, customers, leads }, { screen:"customerDetail", id:custId });
    } catch (e) {
      alert("Save failed: " + (e?.message || e));
    }
  }
  ...
  return (
    <div>
      <div style={{ marginBottom:16, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        <Btn variant="ghost" size="sm" onClick={() => setView({ screen:"dashboard" })}><Icon name="back" size={14} /> Back</Btn>
        <Btn size="sm" onClick={() => { setEditingLead(null); setShowForm(true); }}>+ Log Visit</Btn>
      </div>
      <h2 style={{ fontSize:18, fontWeight:800, color:"#1E3A5F", margin:"0 0 4px" }}>New Business Leads</h2>
      <p style={{ fontSize:13, color:"#6B7280", margin:"0 0 16px" }}>Log the car sales & transport companies you visit, and turn them into a Trade customer the moment they agree.</p>

      {total > 0 && (
        <div style={{ display:"flex", gap:10, marginBottom:16, flexWrap:"wrap" }}>
          <div style={{ flex:1, minWidth:90, background:"#F9FAFB", borderRadius:12, padding:14, border:"1px solid #F3F4F6" }}>
            <div style={{ fontSize:22, fontWeight:800, color:"#1E3A5F" }}>{total}</div>
            <div style={{ fontSize:11, color:"#6B7280", fontWeight:600 }}>VISITED</div>
          </div>
          <div style={{ flex:1, minWidth:90, background:"#ECFDF5", borderRadius:12, padding:14, border:"1px solid #BBF7D0" }}>
            <div style={{ fontSize:22, fontWeight:800, color:"#059669" }}>{converted}</div>
            <div style={{ fontSize:11, color:"#059669", fontWeight:600 }}>CONVERTED</div>
          </div>
          <div style={{ flex:1, minWidth:90, background:"#EFF6FF", borderRadius:12, padding:14, border:"1px solid #BFDBFE" }}>
            <div style={{ fontSize:22, fontWeight:800, color:"#1D4ED8" }}>{rate}%</div>
            <div style={{ fontSize:11, color:"#1D4ED8", fontWeight:600 }}>SUCCESS RATE</div>
          </div>
        </div>
      )}
      ...
      {total > 0 && (
        <div style={{ display:"flex", gap:8, marginBottom:14, flexWrap:"wrap" }}>
          <button onClick={() => setFilter("all")} style={{ background: filter==="all"?"#1E3A5F":"#F3F4F6", color: filter==="all"?"#fff":"#374151", border:"none", borderRadius:8, padding:"6px 12px", fontSize:12, fontWeight:600, cursor:"pointer" }}>All ({total})</button>
          {LEAD_OUTCOMES.map(o => {
            const n = leads.filter(l => l.outcome === o).length;
            if (n === 0) return null;
            return <button key={o} onClick={() => setFilter(o)} style={{ background: filter===o?"#1E3A5F":"#F3F4F6", color: filter===o?"#fff":"#374151", border:"none", borderRadius:8, padding:"6px 12px", fontSize:12, fontWeight:600, cursor:"pointer" }}>{o} ({n})</button>;
          })}
        </div>
      )}

      {filtered.length === 0 && <p style={{ fontSize:13, color:"#9CA3AF", textAlign:"center", marginTop:30 }}>{total === 0 ? "No visits logged yet — tap + Log Visit to get started" : "Nothing matches this filter"}</p>}

      {filtered.map(lead => {
        const colors = LEAD_OUTCOME_COLORS[lead.outcome] || LEAD_OUTCOME_COLORS["Interested"];
        return (
          <Card key={lead.id}>
            <div onClick={() => { setEditingLead(lead); setShowForm(true); }} style={{ cursor:"pointer" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:8 }}>
                <div>
                  <div style={{ fontWeight:700, fontSize:15, color:"#111827" }}>{lead.businessName || "Unnamed"}</div>
                  {lead.contactName && <div style={{ fontSize:13, color:"#6B7280" }}>{lead.contactName}</div>}
                  <div style={{ fontSize:12, color:"#9CA3AF", marginTop:2 }}>{fmtDate(lead.visitDate)}{lead.address ? ` · ${lead.address}` : ""}</div>
                </div>
                <span style={{ fontSize:11, fontWeight:700, color:colors.color, background:colors.bg, padding:"3px 9px", borderRadius:6, whiteSpace:"nowrap" }}>{lead.outcome}</span>
              </div>
              {lead.notes && <div style={{ fontSize:13, color:"#374151", marginTop:8 }}>{lead.notes}</div>}
            </div>
            <div style={{ display:"flex", gap:8, marginTop:10, flexWrap:"wrap" }}>
              {lead.outcome !== "Converted" && (
                <Btn size="sm" onClick={() => convertToCustomer(lead)}>✅ Convert to Trade Customer</Btn>
              )}
              {lead.convertedCustomerId && (
                <Btn size="sm" variant="ghost" onClick={() => setView({ screen:"customerDetail", id:lead.convertedCustomerId })}>View Customer</Btn>
              )}
              <Btn size="sm" variant="danger" onClick={() => deleteLead(lead.id)}><Icon name="trash" size={12} /> Delete</Btn>
            </div>
          </Card>
        );
      })}

      {showForm && <LeadFormModal editLead={editingLead} onSave={saveLead} onClose={() => { setShowForm(false); setEditingLead(null); }} />}
    </div>
  );
}

function LeadFormModal({ editLead, onSave, onClose }) {
  const [businessName, setBusinessName] = useState(editLead?.businessName || "");
  const [contactName, setContactName] = useState(editLead?.contactName || "");
  ...
  const [visitDate, setVisitDate] = useState(editLead?.visitDate || todayISO());
  const [outcome, setOutcome] = useState(editLead?.outcome || "Interested");
  const [notes, setNotes] = useState(editLead?.notes || "");

  function save() {
    if (!businessName.trim()) { alert("Add at least a business name."); return; }
    onSave({
      id: editLead?.id || uid(),
      businessName, contactName, phone, email, address, visitDate, outcome, notes,
      convertedCustomerId: editLead?.convertedCustomerId || null,
      createdAt: editLead?.createdAt || todayISO(),
    });
  }

  return (
    <Modal title={editLead ? "Edit Visit" : "Log a Visit"} onClose={onClose}>
      <Field label="Business Name" required><Input value={businessName} onChange={setBusinessName} placeholder="e.g. Bristol Van Sales" /></Field>
      <Field label="Contact Name"><Input value={contactName} onChange={setContactName} placeholder="Who you spoke to" /></Field>
      <Field label="Phone"><Input value={phone} onChange={setPhone} placeholder="Optional" /></Field>
      <Field label="Email"><Input value={email} onChange={setEmail} placeholder="Optional" /></Field>
      <Field label="Address"><Input value={address} onChange={setAddress} placeholder="Optional" /></Field>
      <Field label="Visit Date"><Input type="date" value={visitDate} onChange={setVisitDate} /></Field>
      <Field label="Outcome"><Select value={outcome} onChange={setOutcome} options={LEAD_OUTCOMES} /></Field>
      <Field label="Notes">
        <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={3}
          style={{ width:"100%", padding:"10px 12px", borderRadius:8, border:"1.5px solid #E5E7EB", fontFamily:"inherit", fontSize:14, resize:"vertical", boxSizing:"border-box" }}
          placeholder="How the conversation went, anything to follow up on…" />
      </Field>
...
function ReportsView({ data }) {
  ...
  // Custom date-range calculator — independent of the monthly chart above
  const [rangeFrom, setRangeFrom] = useState(() => { const d = new Date(now.getFullYear(), now.getMonth(), 1); return d.toISOString().split("T")[0]; });
  ...
  const billed = {}, received = {}, jobCount = {}, repairCount = {};
  months.forEach(m => { billed[m.key]=0; received[m.key]=0; jobCount[m.key]=0; repairCount[m.key]=0; });
  ...
  // A job can cover several repairs (e.g. two chips in one visit) — jobCount counts
  // the visit, repairCount counts every individual repair within it.
  const repairsInJob = (j) => j.repairs?.length ? j.repairs.length : (j.damageType ? 1 : 0);
  (data.jobs || []).forEach(j => {
    if (j.date) {
      const k = j.date.slice(0,7);
      if (k in jobCount) jobCount[k] += 1;
      if (k in repairCount) repairCount[k] += repairsInJob(j);
    }
  });

  // Breakdown of every individual repair (within the selected period) by damage type
  const repairsByType = {};
  DAMAGE_TYPES.forEach(t => { repairsByType[t] = 0; });
  (data.jobs || []).forEach(j => {
    if (!j.date) return;
    const k = j.date.slice(0,7);
    if (!(k in jobCount)) return;
    const reps = j.repairs?.length ? j.repairs : (j.damageType ? [{ type: j.damageType }] : []);
    reps.forEach(r => { const t = r.type || "Chip"; repairsByType[t] = (repairsByType[t] || 0) + 1; });
  });
  const repairsTotal = months.reduce((s,m) => s + repairCount[m.key], 0);
  ...
  const maxRepairs = Math.max(1, ...months.map(m => repairCount[m.key]));
  ...
  const rangeStats = (() => {
    let billedR = 0, receivedR = 0, jobsR = 0, repairsR = 0;
    (data.invoices || []).forEach(inv => {
      const amt = parseFloat(inv.total) || 0;
      const job = (data.jobs || []).find(j => j.id === inv.jobId);
      const billedDate = job?.date || inv.createdAt;
      if (billedDate && billedDate >= rangeFrom && billedDate <= rangeTo) billedR += amt;
      if (inv.paid && inv.paidDate && inv.paidDate >= rangeFrom && inv.paidDate <= rangeTo) receivedR += amt;
    });
    (data.jobs || []).forEach(j => {
      if (j.date && j.date >= rangeFrom && j.date <= rangeTo) { jobsR += 1; repairsR += repairsInJob(j); }
    });
    return { billedR, receivedR, jobsR, repairsR };
  })();
  ...
  return (
    <div>
      ...
      <div style={{ background:"#fff", border:"1px solid #F3F4F6", borderRadius:12, padding:16, marginBottom:20 }}>
        <h3 style={{ margin:"0 0 10px", fontSize:14, fontWeight:700, color:"#374151" }}>Earnings Between Two Dates</h3>
        <div style={{ display:"flex", gap:10, marginBottom:14 }}>
          <div style={{ flex:1 }}>
            <label style={{ display:"block", fontSize:11, fontWeight:700, color:"#9CA3AF", textTransform:"uppercase", marginBottom:4 }}>From</label>
            <input type="date" value={rangeFrom} onChange={e => setRangeFrom(e.target.value)} style={inputStyle} />
            <div style={{ fontSize:12, color:"#6B7280", marginTop:3 }}>{fmtDate(rangeFrom)}</div>
          </div>
          <div style={{ flex:1 }}>
            <label style={{ display:"block", fontSize:11, fontWeight:700, color:"#9CA3AF", textTransform:"uppercase", marginBottom:4 }}>To</label>
            <input type="date" value={rangeTo} onChange={e => setRangeTo(e.target.value)} style={inputStyle} />
            <div style={{ fontSize:12, color:"#6B7280", marginTop:3 }}>{fmtDate(rangeTo)}</div>
          </div>
        </div>
        <div style={{ display:"flex", gap:10 }}>
          <div style={{ flex:1, background:"#EFF6FF", borderRadius:10, padding:12, border:"1px solid #BFDBFE" }}>
            <div style={{ fontSize:18, fontWeight:800, color:"#1D4ED8" }}>£{rangeStats.billedR.toFixed(2)}</div>
            <div style={{ fontSize:11, color:"#1D4ED8", fontWeight:600 }}>Billed</div>
          </div>
          <div style={{ flex:1, background:"#ECFDF5", borderRadius:10, padding:12, border:"1px solid #A7F3D0" }}>
            <div style={{ fontSize:18, fontWeight:800, color:"#059669" }}>£{rangeStats.receivedR.toFixed(2)}</div>
            <div style={{ fontSize:11, color:"#059669", fontWeight:600 }}>Received</div>
          </div>
          <div style={{ flex:1, background:"#F9FAFB", borderRadius:10, padding:12, border:"1px solid #E5E7EB" }}>
            <div style={{ fontSize:18, fontWeight:800, color:"#374151" }}>{rangeStats.jobsR}</div>
            <div style={{ fontSize:11, color:"#6B7280", fontWeight:600 }}>Jobs</div>
          </div>
          <div style={{ flex:1, background:"#FFFBEB", borderRadius:10, padding:12, border:"1px solid #FDE68A" }}>
            <div style={{ fontSize:18, fontWeight:800, color:"#B45309" }}>{rangeStats.repairsR}</div>
            <div style={{ fontSize:11, color:"#B45309", fontWeight:600 }}>Repairs</div>
          </div>
        </div>
        <p style={{ fontSize:11, color:"#9CA3AF", margin:"10px 0 0" }}>"Billed" is by job date (when the work was done); "Received" is by the date each invoice was actually marked paid. "Repairs" counts every individual chip/crack fixed — a job with 2 repairs counts as 2.</p>
      </div>

      {/* Period selector */}
      <div style={{ marginBottom:16, display:"flex", gap:8 }}>
        <select value={period} onChange={e => setPeriod(e.target.value)}
          style={{ flex:1, padding:"12px 14px", borderRadius:8, border:"1.5px solid #E5E7EB", fontSize:15, fontFamily:"inherit", background:"#fff", appearance:"none" }}>
        ...
        {period.startsWith("fin-") && (
          <Btn variant="ghost" onClick={() => openFinancialYearReport(data, parseInt(period.slice(4)))}>📄 PDF</Btn>
        )}
      </div>
      ...
      <div style={{ background:"#fff", borderRadius:12, padding:16, border:"1px solid #F3F4F6", marginBottom:16 }}>
        ...
        <div style={{ display:"flex", fontSize:11, fontWeight:700, color:"#9CA3AF", textTransform:"uppercase", padding:"0 4px 6px", borderBottom:"1px solid #F3F4F6" }}>
          ...
          <div style={{ flex:0.8, textAlign:"right" }}>Repairs</div>
        </div>
        {[...months].reverse().map(m => (
          <div key={m.key} style={{ display:"flex", fontSize:13, padding:"7px 4px", borderBottom:"1px solid #F9FAFB" }}>
            ...
            <div style={{ flex:0.8, textAlign:"right", color:"#B45309" }}>{repairCount[m.key] || 0}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ background:"#fff", borderRadius:12, padding:16, border:"1px solid #F3F4F6", marginBottom:16 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:4 }}>
          <h3 style={{ margin:0, fontSize:14, fontWeight:700, color:"#374151" }}>Repairs · {periodLabel}</h3>
          <span style={{ fontSize:13, fontWeight:800, color:"#B45309" }}>{repairsTotal} total</span>
        </div>
        <p style={{ fontSize:11, color:"#9CA3AF", margin:"0 0 8px" }}>Every individual chip/crack fixed — a job with 2 repairs counts as 2.</p>
        <div style={{ display:"flex", alignItems:"flex-end", gap:3 }}>
          {months.map(m => (
            <div key={m.key} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center" }}>
              <div style={{ fontSize:10, fontWeight:700, color:"#6B7280", marginBottom:2 }}>{repairCount[m.key] || ""}</div>
              <div style={{ width:"60%", height:`${(repairCount[m.key]/maxRepairs)*90}px`, background:"#D97706", borderRadius:"3px 3px 0 0", minHeight: repairCount[m.key]>0?2:0 }} />
              <div style={{ fontSize:9, color:"#9CA3AF", marginTop:4 }}>{m.label}</div>
            </div>
          ))}
        </div>
        <div style={{ display:"flex", gap:8, marginTop:16, flexWrap:"wrap", borderTop:"1px solid #F3F4F6", paddingTop:14 }}>
          {DAMAGE_TYPES.map(type => (
            <div key={type} style={{ flex:"1 1 70px", background:"#FFFBEB", borderRadius:10, padding:"10px 8px", border:"1px solid #FDE68A", textAlign:"center" }}>
              <div style={{ fontSize:16, fontWeight:800, color:"#B45309" }}>{repairsByType[type] || 0}</div>
              <div style={{ fontSize:10, color:"#92400E", fontWeight:600 }}>{type}</div>
            </div>
          ))}
        </div>
      </div>
...
// Computes billed/received/outstanding/jobs/mileage totals (plus a month-by-month
// breakdown, April-to-March) for one financial year — shared by the on-screen Reports
// view and the printable Financial Year PDF.
function computeFYStats(data, fy) {
  const months = [];
  for (let m = 3; m < 12; m++) months.push({ key: `${fy}-${String(m+1).padStart(2,"0")}`, label: new Date(fy, m, 1).toLocaleDateString("en-GB",{month:"short"}) });
  for (let m = 0; m < 3; m++) months.push({ key: `${fy+1}-${String(m+1).padStart(2,"0")}`, label: new Date(fy+1, m, 1).toLocaleDateString("en-GB",{month:"short"}) });
  ...
  (data.jobs || []).forEach(j => { if (j.date) { const k = j.date.slice(0,7); if (k in jobCount) jobCount[k] += 1; } });

  const billedTotal = months.reduce((s,m) => s + billed[m.key], 0);
  const receivedTotal = months.reduce((s,m) => s + received[m.key], 0);
  const jobsTotal = months.reduce((s,m) => s + jobCount[m.key], 0);
  const outstanding = (data.invoices || []).filter(i => !i.paid).reduce((s,i) => {
    const job = (data.jobs || []).find(j => j.id === i.jobId);
    const d = job?.date || i.createdAt;
    return (d && finYearOf(d) === fy) ? s + (parseFloat(i.total)||0) : s;
  }, 0);
  const mileageTotal = (data.mileage || []).filter(e => finYearOf(e.date) === fy).reduce((s,e) => s + (e.miles||0), 0);

  return { months, billed, received, jobCount, billedTotal, receivedTotal, jobsTotal, outstanding, mileageTotal };
}

// Pure, synchronous — a printable financial year summary, showing the selected year
// alongside the previous year for comparison. Includes a monthly breakdown for each.
function openFinancialYearReport(data, fy) {
  const thisYear = computeFYStats(data, fy);
  const lastYear = computeFYStats(data, fy - 1);
  const fmtD = new Date().toLocaleDateString("en-GB");

  const yearBlock = (stats, label) => `
    <div style="border:1px solid #E5E7EB;border-radius:10px;padding:16px;margin-bottom:16px;page-break-inside:avoid;">
      <div style="font-size:14px;font-weight:800;color:#1E3A5F;margin-bottom:10px;">${label}</div>
      <div style="display:flex;gap:10px;margin-bottom:14px;flex-wrap:wrap;">
        <div style="flex:1;min-width:100px;background:#EFF6FF;border-radius:8px;padding:10px;"><div style="font-size:16px;font-weight:800;color:#1D4ED8;">£${stats.billedTotal.toFixed(2)}</div><div style="font-size:10px;color:#1D4ED8;font-weight:600;">BILLED</div></div>
        <div style="flex:1;min-width:100px;background:#ECFDF5;border-radius:8px;padding:10px;"><div style="font-size:16px;font-weight:800;color:#059669;">£${stats.receivedTotal.toFixed(2)}</div><div style="font-size:10px;color:#059669;font-weight:600;">RECEIVED</div></div>
        <div style="flex:1;min-width:100px;background:#FEF2F2;border-radius:8px;padding:10px;"><div style="font-size:16px;font-weight:800;color:#DC2626;">£${stats.outstanding.toFixed(2)}</div><div style="font-size:10px;color:#DC2626;font-weight:600;">OUTSTANDING</div></div>
        <div style="flex:1;min-width:100px;background:#F9FAFB;border-radius:8px;padding:10px;"><div style="font-size:16px;font-weight:800;color:#374151;">${stats.jobsTotal}</div><div style="font-size:10px;color:#6B7280;font-weight:600;">JOBS</div></div>
        <div style="flex:1;min-width:100px;background:#F9FAFB;border-radius:8px;padding:10px;"><div style="font-size:16px;font-weight:800;color:#374151;">${stats.mileageTotal.toLocaleString()}</div><div style="font-size:10px;color:#6B7280;font-weight:600;">MILES</div></div>
      </div>
      <table style="width:100%;border-collapse:collapse;">
        <thead><tr style="border-bottom:1px solid #E5E7EB;">
          <th style="text-align:left;padding:5px 4px;font-size:10px;color:#9CA3AF;text-transform:uppercase;">Month</th>
          <th style="text-align:right;padding:5px 4px;font-size:10px;color:#9CA3AF;text-transform:uppercase;">Billed</th>
          <th style="text-align:right;padding:5px 4px;font-size:10px;color:#9CA3AF;text-transform:uppercase;">Received</th>
          <th style="text-align:right;padding:5px 4px;font-size:10px;color:#9CA3AF;text-transform:uppercase;">Jobs</th>
        </tr></thead>
        <tbody>
          ${stats.months.map(m => `
          <tr style="border-bottom:1px solid #F9FAFB;">
            <td style="padding:5px 4px;font-size:12px;font-weight:600;color:#111827;">${m.label}</td>
            <td style="padding:5px 4px;font-size:12px;text-align:right;color:#1D4ED8;">£${stats.billed[m.key].toFixed(0)}</td>
            <td style="padding:5px 4px;font-size:12px;text-align:right;color:#059669;">£${stats.received[m.key].toFixed(0)}</td>
            <td style="padding:5px 4px;font-size:12px;text-align:right;color:#6B7280;">${stats.jobCount[m.key]}</td>
          </tr>`).join("")}
        </tbody>
      </table>
    </div>`;
  ...
  const html = `<!DOCTYPE html>
<html><head><script>try{window.opener=null;}catch(e){}</script><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Financial Year Report</title>
<style>
  body { margin:0; padding:0; background:#F8FAFC; font-family:Arial,sans-serif; }
  @media print { .no-print { display:none !important; } body { background:#fff; } }
</style></head><body>
<div class="no-print" style="position:sticky;top:0;z-index:100;background:#1E3A5F;padding:12px 16px;display:flex;gap:10px;align-items:center;justify-content:center;flex-wrap:wrap;">
  <div style="font-size:13px;color:#93C5FD;font-weight:600;width:100%;text-align:center;">Tap Save as PDF</div>
  <button onclick="window.print()" style="background:#F59E0B;color:#fff;border:none;border-radius:8px;padding:10px 20px;font-size:14px;font-weight:700;cursor:pointer;">💾 Save as PDF</button>
  <button onclick="window.close()" style="background:rgba(255,255,255,.15);color:#fff;border:1.5px solid rgba(255,255,255,.4);border-radius:8px;padding:10px 20px;font-size:14px;font-weight:700;cursor:pointer;">✕ Close</button>
</div>
<div style="max-width:700px;margin:0 auto;padding:24px;background:#fff;">
  ...
  <div style="font-size:16px;font-weight:800;color:#1E3A5F;margin-bottom:4px;">Financial Year Report</div>
  <div style="font-size:12px;color:#9CA3AF;margin-bottom:18px;">Generated ${fmtD}. "Billed" is by job date; "Received" is by the date each invoice was marked paid.</div>
  ${yearBlock(thisYear, `FY ${fy}/${(fy+1).toString().slice(2)} (Apr ${fy} – Mar ${fy+1})`)}
  ${yearBlock(lastYear, `FY ${fy-1}/${fy.toString().slice(2)} (Apr ${fy-1} – Mar ${fy}) — previous year`)}
</div>
</body></html>`;
  const win = window.open("about:blank", "_blank");
  if (win) { win.document.open(); win.document.write(html); win.document.close(); }
...
// Pure, synchronous — a printable mileage log, oldest first within the report, showing
// whichever entries are currently in view (respects the year filter on screen) plus a total.
function openMileageReport(entries, yearLabel) {
  const fmtD = new Date().toLocaleDateString("en-GB");
  const sorted = [...entries].sort((a,b) => a.date.localeCompare(b.date));
  const total = sorted.reduce((s,e) => s + (e.miles||0), 0);

  const rows = sorted.map(e => `
    <tr>
      <td style="padding:8px 10px;border-bottom:1px solid #E5E7EB;font-size:13px;color:#111827;">${fmtDate(e.date)}</td>
      <td style="padding:8px 10px;border-bottom:1px solid #E5E7EB;font-size:13px;color:#6B7280;">${e.note||""}</td>
      <td style="padding:8px 10px;border-bottom:1px solid #E5E7EB;font-size:13px;color:#111827;text-align:right;font-weight:700;">${e.miles.toLocaleString()}</td>
    </tr>`).join("");

  const html = `<!DOCTYPE html>
<html><head><script>try{window.opener=null;}catch(e){}</script><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Mileage Log</title>
<style>
  body { margin:0; padding:0; background:#F8FAFC; font-family:Arial,sans-serif; }
  @media print { .no-print { display:none !important; } body { background:#fff; } }
</style></head><body>
...
<div style="max-width:700px;margin:0 auto;padding:24px;background:#fff;">
  <div style="display:flex;align-items:center;gap:14px;border-bottom:3px solid #F59E0B;padding-bottom:14px;margin-bottom:18px;">
    ...
    <div>
      <div style="font-size:20px;font-weight:800;color:#1E3A5F;">Windscreen Repairs (Bristol)</div>
      <div style="font-size:12px;color:#6B7280;">David Morgan trading as Windscreen Repairs (Bristol)</div>
    </div>
  </div>
  <div style="font-size:16px;font-weight:800;color:#1E3A5F;margin-bottom:4px;">Mileage Log — ${yearLabel}</div>
  <div style="font-size:12px;color:#9CA3AF;margin-bottom:18px;">Generated ${fmtD} · ${sorted.length} entr${sorted.length===1?"y":"ies"}</div>
  <table style="width:100%;border-collapse:collapse;border:1px solid #E5E7EB;">
    <thead><tr style="background:#F9FAFB;">
      <th style="padding:8px 10px;text-align:left;font-size:11px;color:#6B7280;text-transform:uppercase;">Date</th>
      <th style="padding:8px 10px;text-align:left;font-size:11px;color:#6B7280;text-transform:uppercase;">Note</th>
      <th style="padding:8px 10px;text-align:right;font-size:11px;color:#6B7280;text-transform:uppercase;">Miles</th>
    </tr></thead>
    <tbody>${rows || '<tr><td colspan="3" style="padding:14px;color:#9CA3AF;font-size:13px;">No entries</td></tr>'}</tbody>
  </table>
  <div style="display:flex;justify-content:space-between;align-items:center;background:#EFF6FF;border:1px solid #BFDBFE;border-radius:8px;padding:14px 16px;margin-top:16px;">
    <span style="font-size:14px;font-weight:700;color:#1D4ED8;">Total Miles</span>
    <span style="font-size:20px;font-weight:800;color:#1D4ED8;">${total.toLocaleString()}</span>
  </div>
</div>
</body></html>`;
  const win2 = window.open("about:blank", "_blank");
  if (win2) { win2.document.open(); win2.document.write(html); win2.document.close(); }
  else { alert("Please allow pop-ups for this site, then try again."); }
}

// Pure, synchronous — a sample Damage Report using fictional vehicles, so it can be sent
// to any customer as an example of what the real thing looks like, before any actual
// inspection has taken place. Clearly labelled as a sample throughout.
function openDemoDamageReport() {
  const logoUrl = LOGO_DATA_URI;
  const fmtD = new Date().toLocaleDateString("en-GB");
  const sample = [
    { car: "AB12 CDE · Ford Transit", damage: "Chip · Drivers Side · Top Right" },
    { car: "", damage: "Chip · Passenger Side · Centre" },
    { car: "CD34 EFG · Mercedes Sprinter", damage: "Chip · Drivers Side · Bottom Right" },
    { car: "EF56 GHI · Vauxhall Vivaro", damage: "Crack · Passenger Side · Top Left" },
  ];
  const rows = sample.map((d, i) => `
    <tr>
      <td style="padding:10px 12px;${d.car && i>0 ? 'border-top:2px solid #9CA3AF;' : ''}border-bottom:1px solid #E5E7EB;font-size:13px;color:#6B7280;">${i+1}</td>
      <td style="padding:10px 12px;${d.car && i>0 ? 'border-top:2px solid #9CA3AF;' : ''}border-bottom:1px solid #E5E7EB;font-size:13px;color:#111827;">${d.car ? `<b>${d.car}</b>` : ""}</td>
      <td style="padding:10px 12px;${d.car && i>0 ? 'border-top:2px solid #9CA3AF;' : ''}border-bottom:1px solid #E5E7EB;font-size:13px;color:#111827;">${d.damage}</td>
      <td style="padding:10px 12px;${d.car && i>0 ? 'border-top:2px solid #9CA3AF;' : ''}border-bottom:1px solid #E5E7EB;text-align:center;"><span style="display:inline-block;width:16px;height:16px;border:2px solid #374151;border-radius:3px;"></span></td>
    </tr>`).join("");

  const html = `<!DOCTYPE html>
<html><head><script>try{window.opener=null;}catch(e){}</script><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Sample Damage Report</title>
<style>
  body { margin:0; padding:0; background:#F8FAFC; font-family:Arial,sans-serif; }
  @media print { .no-print { display:none !important; } body { background:#fff; } }
</style></head><body>
...
<div style="max-width:700px;margin:0 auto;padding:24px;background:#fff;">
  <div style="text-align:center;background:#FFFBEB;border:1px solid #FDE68A;border-radius:8px;padding:8px 12px;margin-bottom:16px;">
    <span style="font-size:12px;font-weight:800;color:#92400E;letter-spacing:.05em;">SAMPLE — FOR DEMONSTRATION ONLY</span>
  </div>
  <div style="display:flex;align-items:center;gap:14px;border-bottom:3px solid #F59E0B;padding-bottom:14px;margin-bottom:18px;">
    <img src="${logoUrl}" style="width:56px;height:56px;object-fit:contain;" />
    <div>
  ...
  <div style="font-size:13px;color:#6B7280;margin-bottom:2px;">Prepared for: <b style="color:#111827;">Example Fleet Ltd (sample)</b></div>
  <div style="font-size:13px;color:#6B7280;margin-bottom:14px;">Date: ${fmtD}</div>
  <div style="font-size:13px;color:#374151;line-height:1.5;margin-bottom:16px;">This is an example of the report you'll receive after we inspect your vehicles — every chip or crack found is listed individually against the vehicle it's on, with a box to tick for each one you'd like repaired.</div>
  <table style="width:100%;border-collapse:collapse;border:1px solid #E5E7EB;">
    <thead><tr style="background:#F9FAFB;">
  ...
  <div style="font-size:12px;color:#9CA3AF;margin:20px 0 24px;">3 vehicles, 4 damage items listed (sample data) · Windscreen Repairs (Bristol)</div>
  <div style="border-top:1px solid #E5E7EB;padding-top:16px;">
    <div style="font-size:12px;color:#6B7280;margin-bottom:14px;">On a real report, you'd tick above the damage you'd like repaired, then complete below to authorise the work.</div>
    <div style="display:flex;gap:24px;flex-wrap:wrap;margin-bottom:4px;">
      <div style="flex:1;min-width:180px;">
        <div style="font-size:11px;color:#9CA3AF;text-transform:uppercase;letter-spacing:.04em;margin-bottom:16px;">Authorised by (name)</div>
        <div style="border-bottom:1px solid #9CA3AF;height:6px;"></div>
  ...
  const win3 = window.open("about:blank", "_blank");
  if (win3) { win3.document.open(); win3.document.write(html); win3.document.close(); }
  else { alert("Please allow pop-ups for this site, then try again."); }
}

function openShowcaseWindow(jobs) {
  const cards = jobs.map(j => {
    const beforeImgs = (j.photosBefore||[]).map(p => `<img src="${p.url||p.pending}" style="width:100%;aspect-ratio:4/3;object-fit:cover;border-radius:8px;" />`).join("");
    const afterImgs  = (j.photosAfter||[]).map(p => `<img src="${p.url||p.pending}" style="width:100%;aspect-ratio:4/3;object-fit:cover;border-radius:8px;" />`).join("");
    return `
    <div style="border:1px solid #E5E7EB;border-radius:12px;padding:16px;margin-bottom:18px;page-break-inside:avoid;">
      <div style="font-size:13px;font-weight:700;color:#1E3A5F;margin-bottom:10px;">${j.jobType || "Windscreen Repair"}</div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
        <div>
          <div style="font-size:10px;font-weight:700;color:#9CA3AF;text-transform:uppercase;margin-bottom:6px;">Before</div>
          <div style="display:grid;grid-template-columns:${beforeImgs.split('<img').length-1 > 1 ? '1fr 1fr' : '1fr'};gap:6px;">${beforeImgs || '<div style="color:#D1D5DB;font-size:12px;">No photo</div>'}</div>
        </div>
        <div>
          <div style="font-size:10px;font-weight:700;color:#9CA3AF;text-transform:uppercase;margin-bottom:6px;">After</div>
          <div style="display:grid;grid-template-columns:${afterImgs.split('<img').length-1 > 1 ? '1fr 1fr' : '1fr'};gap:6px;">${afterImgs || '<div style="color:#D1D5DB;font-size:12px;">No photo</div>'}</div>
        </div>
      </div>
    </div>`;
  }).join("");
  ...
  const html = `<!DOCTYPE html>
<html><head><script>try{window.opener=null;}catch(e){}</script><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Our Work</title>
<style>
  body { margin:0; padding:0; background:#F8FAFC; font-family:Arial,sans-serif; }
  @media print { .no-print { display:none !important; } body { background:#fff; } }
</style></head><body>
<div class="no-print" style="position:sticky;top:0;z-index:100;background:#1E3A5F;padding:12px 16px;display:flex;gap:10px;align-items:center;justify-content:center;flex-wrap:wrap;">
  <div style="font-size:13px;color:#93C5FD;font-weight:600;width:100%;text-align:center;">Tap Save as PDF, then attach to an email</div>
  <button onclick="window.print()" style="background:#F59E0B;color:#fff;border:none;border-radius:8px;padding:10px 20px;font-size:14px;font-weight:700;cursor:pointer;">💾 Save as PDF</button>
  <a href="mailto:?subject=${encodeURIComponent("Windscreen Repairs Bristol — Our Work")}" style="background:#fff;color:#1E3A5F;border:none;border-radius:8px;padding:10px 20px;font-size:14px;font-weight:700;cursor:pointer;text-decoration:none;">✉️ Open Mail App</a>
  <button onclick="window.close()" style="background:rgba(255,255,255,.15);color:#fff;border:1.5px solid rgba(255,255,255,.4);border-radius:8px;padding:10px 20px;font-size:14px;font-weight:700;cursor:pointer;">✕ Close</button>
</div>
<div style="max-width:700px;margin:0 auto;padding:24px;background:#fff;">
  <div style="display:flex;align-items:center;gap:14px;border-bottom:3px solid #F59E0B;padding-bottom:14px;margin-bottom:18px;">
  ...
  <div style="font-size:16px;font-weight:800;color:#1E3A5F;margin-bottom:4px;">Our Work</div>
  <div style="font-size:13px;color:#6B7280;margin-bottom:18px;">A few examples of the quality you can expect.</div>
  ${cards || '<p style="color:#9CA3AF;font-size:13px;">No jobs selected.</p>'}
</div>
</body></html>`;
  const win = window.open("about:blank", "_blank");
  if (win) { win.document.open(); win.document.write(html); win.document.close(); }
...
function PhotoShowcaseView({ data, setView }) {
  const jobsWithPhotos = data.jobs
    .filter(j => j.photosBefore?.length === 1 && j.photosAfter?.length === 1)
    .sort((a,b) => (b.date||"").localeCompare(a.date||""));

  const [selected, setSelected] = useState(() => {
    const s = {};
    jobsWithPhotos.forEach(j => { s[j.id] = true; });
    return s;
  });
  const toggle = (id) => setSelected(s => ({ ...s, [id]: !s[id] }));
  const count = Object.values(selected).filter(Boolean).length;
  ...
  return (
    <div>
      ...
      <h2 style={{ fontSize:18, fontWeight:800, color:"#1E3A5F", margin:"0 0 6px" }}>Photo Showcase</h2>
      <p style={{ fontSize:13, color:"#6B7280", margin:"0 0 16px" }}>Jobs with a clean, single before and after photo — all ticked by default, untick anything that doesn't look its best.</p>

      {jobsWithPhotos.length === 0 && <Card><p style={{ margin:0, color:"#9CA3AF", fontSize:14, textAlign:"center" }}>No jobs with a single before and after photo yet</p></Card>}

      {jobsWithPhotos.map(j => {
        const vehicle = j.vehicleId ? data.vehicles.find(v => v.id === j.vehicleId) : null;
        return (
          <label key={j.id} style={{ display:"block", border:"1px solid #F3F4F6", borderRadius:10, padding:12, marginBottom:8, background: selected[j.id] ? "#EFF6FF" : "#fff", cursor:"pointer" }}>
            <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:8 }}>
              <input type="checkbox" checked={!!selected[j.id]} onChange={() => toggle(j.id)} style={{ width:18, height:18 }} />
              <div style={{ flex:1 }}>
                <div style={{ fontSize:14, fontWeight:700, color:"#111827" }}>{fmtDate(j.date)} · {j.jobType}</div>
                {vehicle && <div style={{ fontSize:12, color:"#9CA3AF" }}>{vehicle.make} {vehicle.model}</div>}
              </div>
            </div>
            <div style={{ display:"flex", gap:6 }}>
              <img src={j.photosBefore[0].url||j.photosBefore[0].pending} style={{ width:64, height:64, objectFit:"cover", borderRadius:6, border:"2px solid #DC2626" }} />
              <img src={j.photosAfter[0].url||j.photosAfter[0].pending} style={{ width:64, height:64, objectFit:"cover", borderRadius:6, border:"2px solid #059669" }} />
            </div>
          </label>
        );
      })}
      ...
      {jobsWithPhotos.length > 0 && (
        <Btn onClick={() => openShowcaseWindow(jobsWithPhotos.filter(j => selected[j.id]))} disabled={count===0} style={{ width:"100%", justifyContent:"center", marginTop:10 }}>
          📸 Generate Showcase ({count})
        </Btn>
      )}
    </div>
  );
...
function MileageView({ data, setView }) {
  ...
  const [stops, setStops] = useState(null); // null | array of { postcode, jobs, customers } — ordered, reorderable
  const [routeResult, setRouteResult] = useState(null); // null | "loading" | { legs, total } | "error"

  function findStops() {
    const jobsThatDay = data.jobs.filter(j => j.date === estimateDate && j.locPostcode);
    if (jobsThatDay.length === 0) { setStops("none"); return; }

    // Group by postcode (normalised) — several vehicles at the same address on the
    // same day is one stop, not one per vehicle.
    const groups = {};
    jobsThatDay.forEach(j => {
      const key = j.locPostcode.replace(/\s+/g, "").toUpperCase();
      if (!groups[key]) groups[key] = { postcode: j.locPostcode, jobs: [] };
      groups[key].jobs.push(j);
    });
    const list = Object.values(groups).map(g => ({
      postcode: g.postcode, jobs: g.jobs,
      customers: [...new Set(g.jobs.map(j => j.customerId))].map(cid => data.customers.find(c => c.id === cid)),
      earliestTime: g.jobs.map(j => j.jobTime).filter(Boolean).sort()[0] || null,
    }));
    // Default order: by job time where set, otherwise left as found
    list.sort((a,b) => (a.earliestTime || "99:99").localeCompare(b.earliestTime || "99:99"));
    setStops(list);
    setRouteResult(null);
  }
  ...
  async function calculateRoute() {
    setRouteResult("loading");
    const result = await estimateRouteMiles(stops.map(s => s.postcode));
    setRouteResult(result || "error");
  }
  async function addRouteToLog() {
    if (!routeResult || typeof routeResult !== "object") return;
    const names = [...new Set(stops.flatMap(s => s.customers.map(c => c?.company || c?.companyContact || "Job")))].join(" → ");
    const mileage = [...(data.mileage || []), { id: uid(), date: estimateDate, miles: routeResult.total, note: `${names} (estimated route from Cheddar)`, createdAt: todayISO() }];
    await saveAndReload({ ...data, mileage });
  }
  ...
  async function add() {
    ...
    let mileage;
    if (editingId) {
      mileage = (data.mileage || []).map(e => e.id === editingId ? { ...e, date, miles: m, note } : e);
    } else {
      mileage = [...(data.mileage || []), { id: uid(), date, miles: m, note, createdAt: todayISO() }];
    }
    setMiles(""); setNote(""); setDate(todayISO()); setEditingId(null);
    await saveAndReload({ ...data, mileage });
  }
  function startEdit(e) {
    setEditingId(e.id); setDate(e.date); setMiles(String(e.miles)); setNote(e.note || "");
  }
  function cancelEdit() {
    setEditingId(null); setDate(todayISO()); setMiles(""); setNote("");
  }
  async function remove(id) {
  ...
  return (
    <div>
      ...
      <Card style={{ marginBottom:16 }}>
        <div style={{ fontSize:13, fontWeight:700, color:"#1E3A5F", marginBottom:4 }}>📍 Estimate from a Day's Jobs</div>
        <p style={{ fontSize:12, color:"#6B7280", margin:"0 0 10px" }}>Works out mileage for the actual route — Cheddar to your first job, job to job, then back to Cheddar at the end — not a separate trip home in between each one.</p>
        <Field label="Date"><Input type="date" value={estimateDate} onChange={v => { setEstimateDate(v); setStops(null); setRouteResult(null); }} /></Field>
        {stops === null && <Btn variant="ghost" onClick={findStops} style={{ width:"100%", justifyContent:"center" }}>Find That Day's Stops</Btn>}
        {stops === "none" && <p style={{ fontSize:13, color:"#9CA3AF", textAlign:"center", margin:0 }}>No jobs with a postcode found on that date.</p>}

        {Array.isArray(stops) && (
          <>
            <div style={{ fontSize:11, fontWeight:700, color:"#6B7280", textTransform:"uppercase", marginBottom:8 }}>Stops, in order — check this matches your actual route</div>
            {stops.map((s, i) => (
              <div key={s.postcode} style={{ display:"flex", alignItems:"center", gap:8, padding:"8px 10px", border:"1px solid #F3F4F6", borderRadius:8, marginBottom:5 }}>
                <span style={{ fontSize:13, fontWeight:800, color:"#1E3A5F", width:18 }}>{i+1}</span>
                <div style={{ flex:1 }}>
                  <div style={{ fontSize:13, color:"#111827" }}>{s.customers.map(c => c?.company || c?.companyContact || "Job").join(", ")}</div>
                  <div style={{ fontSize:11, color:"#9CA3AF" }}>{s.postcode}{s.jobs.length > 1 ? ` · ${s.jobs.length} vehicles` : ""}{s.earliestTime ? ` · ${s.earliestTime}` : ""}</div>
                </div>
                <div style={{ display:"flex", flexDirection:"column", gap:2 }}>
                  <button onClick={() => moveStop(i, -1)} disabled={i===0} style={{ background:"#F3F4F6", border:"none", borderRadius:5, width:26, height:20, fontSize:11, cursor: i===0?"default":"pointer", opacity: i===0?0.4:1 }}>▲</button>
                  <button onClick={() => moveStop(i, 1)} disabled={i===stops.length-1} style={{ background:"#F3F4F6", border:"none", borderRadius:5, width:26, height:20, fontSize:11, cursor: i===stops.length-1?"default":"pointer", opacity: i===stops.length-1?0.4:1 }}>▼</button>
                </div>
              </div>
            ))}
            <p style={{ fontSize:11, color:"#9CA3AF", margin:"4px 0 10px" }}>Defaulted to job time order where set — use ▲▼ to fix any that are wrong.</p>

            {routeResult === null && <Btn onClick={calculateRoute} style={{ width:"100%", justifyContent:"center" }}>Calculate Route</Btn>}
            {routeResult === "loading" && <p style={{ fontSize:13, color:"#9CA3AF", textAlign:"center", margin:0 }}>Working it out…</p>}
            {routeResult && typeof routeResult === "object" && (
              <>
                {routeResult.failed?.length > 0 && (
                  <p style={{ fontSize:12, color:"#DC2626", background:"#FEF2F2", border:"1px solid #FEE2E2", borderRadius:8, padding:"8px 10px", marginBottom:10 }}>
                    ⚠️ Couldn't find: {routeResult.failed.join(", ")} — check that postcode is correct on the job, or add mileage for that leg manually. {routeResult.legs.length > 0 ? "The total below only covers the rest of the route." : ""}
                  </p>
                )}
                {routeResult.legs.length > 0 && (
                  <>
                    ...
                    <div style={{ display:"flex", justifyContent:"space-between", fontSize:14, fontWeight:800, color:"#1E3A5F", borderTop:"1px solid #E5E7EB", marginTop:6, paddingTop:8, marginBottom:10 }}>
                      <span>Total</span><span>{routeResult.total} miles</span>
                    </div>
                    <Btn onClick={addRouteToLog} style={{ width:"100%", justifyContent:"center" }}>Add {routeResult.total} miles to Mileage Log</Btn>
                    <p style={{ fontSize:11, color:"#9CA3AF", margin:"8px 0 0", textAlign:"center" }}>Estimated from postcode data, not an exact route — a reasonable figure, not a guarantee.</p>
                  </>
                )}
              </>
            )}
      ...
      <Card>
        <div style={{ fontSize:13, fontWeight:700, color:"#1E3A5F", marginBottom:10 }}>{editingId ? "Edit Mileage Entry" : "Add Mileage"}</div>
        ...
        <div style={{ display:"flex", gap:8 }}>
          <Btn onClick={add} style={{ flex:1, justifyContent:"center" }} disabled={!miles}>{editingId ? "Save Changes" : "Add"}</Btn>
          {editingId && <Btn variant="ghost" onClick={cancelEdit} style={{ justifyContent:"center" }}>Cancel</Btn>}
        </div>
      </Card>
      ...
      <div style={{ marginTop:16 }}>
        {allEntries.length > 0 && (
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10, gap:8 }}>
            ...
            <div style={{ display:"flex", gap:8 }}>
              ...
              <Btn size="sm" variant="ghost" onClick={() => openMileageReport(entries, yearFilter === "all" ? "All Years" : `FY ${yearFilter}/${(Number(yearFilter)+1).toString().slice(2)}`)}>📄 PDF</Btn>
            </div>
          </div>
        )}
        {yearFilter !== "all" && (
        ...
        {entries.map(e => (
          <Card key={e.id} style={e.id===editingId ? { background:"#EFF6FF", borderColor:"#BFDBFE" } : undefined}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", gap:8 }}>
              <div>
              ...
              <div style={{ display:"flex", gap:6 }}>
                <button onClick={() => startEdit(e)} style={{ background:"#EFF6FF", color:"#1E3A5F", border:"none", borderRadius:6, padding:"6px 12px", fontSize:13, fontWeight:600, cursor:"pointer" }}>Edit</button>
  ...
  return (
    <div className="crm-shell" style={{ fontFamily:"'Inter',system-ui,sans-serif", background:"#F8FAFC", minHeight:"100vh", margin:"0 auto" }}>
      ...
      <div style={{ padding:"16px 16px 110px" }}>
        ...
        {view.screen==="settingsTerms"    && <SettingsTermsView    data={data} setView={setView} />}
        {view.screen==="settingsOffers"   && <SettingsOffersView   data={data} setView={setView} />}
        {view.screen==="settingsBalances" && <SettingsBalancesView data={data} setView={setView} />}
        {view.screen==="settingsPricing"  && <SettingsPricingView  data={data} setView={setView} />}
        {view.screen==="settingsTimeOff"  && <SettingsTimeOffView  data={data} setView={setView} />}
