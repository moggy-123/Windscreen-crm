...
const BUILD_NUMBER = "B65 · 18 Jul 2026";
...
function PhotoShowcaseView({ data, setView }) {
  const jobsWithPhotos = data.jobs
    .filter(j => (j.photosBefore?.length > 0) || (j.photosAfter?.length > 0))
    .sort((a,b) => (b.date||"").localeCompare(a.date||""));

  const [selected, setSelected] = useState(() => {
    const s = {};
    jobsWithPhotos.forEach(j => { s[j.id] = (j.photosBefore?.length > 0) && (j.photosAfter?.length > 0); });
    return s;
  });
  const toggle = (id) => setSelected(s => ({ ...s, [id]: !s[id] }));
  const count = Object.values(selected).filter(Boolean).length;
  ...
  return (
    <div>
      ...
      <p style={{ fontSize:13, color:"#6B7280", margin:"0 0 16px" }}>Tick the before/after pairs worth showing off — unselect anything that doesn't look its best. Jobs with a full before-and-after pair are ticked by default; the rest are left for you to decide.</p>

      {jobsWithPhotos.length === 0 && <Card><p style={{ margin:0, color:"#9CA3AF", fontSize:14, textAlign:"center" }}>No jobs with photos yet</p></Card>}
      ...
      {jobsWithPhotos.map(j => {
        ...
        return (
          <label key={j.id} style={{ display:"block", border:"1px solid #F3F4F6", borderRadius:10, padding:12, marginBottom:8, background: selected[j.id] ? "#EFF6FF" : "#fff", cursor:"pointer" }}>
            <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom: (j.photosBefore?.length||j.photosAfter?.length) ? 8 : 0 }}>
              ...
              {(j.photosBefore?.length>0) && (j.photosAfter?.length>0)
                ? <span style={{ fontSize:10, fontWeight:700, color:"#059669", background:"#ECFDF5", padding:"3px 7px", borderRadius:6 }}>FULL PAIR</span>
                : <span style={{ fontSize:10, fontWeight:700, color:"#D97706", background:"#FFFBEB", padding:"3px 7px", borderRadius:6 }}>PARTIAL</span>}
            </div>
            <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
              {(j.photosBefore||[]).slice(0,3).map(p => <img key={p.id||p.url} src={p.url||p.pending} style={{ width:56, height:56, objectFit:"cover", borderRadius:6, border:"2px solid #DC2626" }} />)}
              {(j.photosAfter||[]).slice(0,3).map(p => <img key={p.id||p.url} src={p.url||p.pending} style={{ width:56, height:56, objectFit:"cover", borderRadius:6, border:"2px solid #059669" }} />)}
            </div>
          </label>
        );
      })}
