    ...
    for (const table of tables) {
      ...
      for (const rec of (cloud[table]||[])) {
        if (!backupIds.has(rec.id)) {
          try { await deleteRecord(table, rec.id); } catch { failCount++; failedTables.add(table); }
        }
      }
      for (const rec of (backup[table]||[])) {
        try { await pushOne(table, rec); } catch (e) { failCount++; failedTables.add(table); console.warn(`Restore: failed on ${table}`, rec.id, e); }
      }
    }
