...
const BUILD_NUMBER = "B78 · 18 Jul 2026";
...
function MileageView({ data, setView }) {
  ...
  const [estimateResult, setEstimateResult] = useState(null); // null | "loading" | { lines, total } | "error" | "none"

  async function runEstimate() {
    const jobsThatDay = data.jobs.filter(j => j.date === estimateDate && j.locPostcode);
    if (jobsThatDay.length === 0) { setEstimateResult("none"); return; }
    setEstimateResult("loading");

    // Group by postcode (normalised) — several vehicles at the same address on the
    // same day is one trip there and back, not one trip per vehicle.
    const groups = {};
    ...
    for (const key of Object.keys(groups)) {
      ...
      const miles = await estimateRoundTripMiles(group.postcode);
      const customers = [...new Set(group.jobs.map(j => j.customerId))].map(cid => data.customers.find(c => c.id === cid));
      lines.push({ postcode: group.postcode, jobs: group.jobs, customers, miles });
    }
    if (lines.every(l => l.miles === null)) { setEstimateResult("error"); return; }
    const total = lines.reduce((s,l) => s + (l.miles || 0), 0);
    setEstimateResult({ lines, total });
  }
  async function addEstimateToLog() {
    if (!estimateResult || typeof estimateResult !== "object") return;
    const names = [...new Set(estimateResult.lines.flatMap(l => l.customers.map(c => c?.company || c?.companyContact || "Job")))].join(", ");
    const mileage = [...(data.mileage || []), { id: uid(), date: estimateDate, miles: estimateResult.total, note: `${names} (estimated round trips from Cheddar)`, createdAt: todayISO() }];
    await saveAndReload({ ...data, mileage });
  }
  ...
  return (
    <div>
      ...
      <Card style={{ marginBottom:16 }}>
        ...
        <p style={{ fontSize:12, color:"#6B7280", margin:"0 0 10px" }}>Works out round-trip mileage from Cheddar to each job's postcode and back, for a chosen day, and adds it as one entry.</p>
        <Field label="Date"><Input type="date" value={estimateDate} onChange={v => { setEstimateDate(v); setEstimateResult(null); }} /></Field>
        {estimateResult === null && <Btn variant="ghost" onClick={runEstimate} style={{ width:"100%", justifyContent:"center" }}>Estimate</Btn>}
        {estimateResult === "loading" && <p style={{ fontSize:13, color:"#9CA3AF", textAlign:"center", margin:0 }}>Working it out…</p>}
        {estimateResult === "none" && <p style={{ fontSize:13, color:"#9CA3AF", textAlign:"center", margin:0 }}>No jobs with a postcode found on that date.</p>}
        {estimateResult === "error" && <p style={{ fontSize:13, color:"#DC2626", textAlign:"center", margin:0 }}>Couldn't estimate any of that day's postcodes — check they're valid, or add the mileage manually.</p>}
        {estimateResult && typeof estimateResult === "object" && (
          <>
            {estimateResult.lines.map((l, i) => (
              <div key={i} style={{ padding:"6px 0", borderBottom: i < estimateResult.lines.length-1 ? "1px solid #F3F4F6" : "none" }}>
                <div style={{ display:"flex", justifyContent:"space-between", fontSize:13, color:"#374151" }}>
                  <span>{l.customers.map(c => c?.company || c?.companyContact || "Job").join(", ")} — {l.postcode}</span>
                  <span>{l.miles === null ? "couldn't estimate" : `${l.miles} mi`}</span>
                </div>
                {l.jobs.length > 1 && <div style={{ fontSize:11, color:"#9CA3AF" }}>{l.jobs.length} vehicles, one trip</div>}
              </div>
            ))}
            <div style={{ display:"flex", justifyContent:"space-between", fontSize:14, fontWeight:800, color:"#1E3A5F", borderTop:"1px solid #E5E7EB", marginTop:6, paddingTop:8, marginBottom:10 }}>
              <span>Total</span><span>{estimateResult.total} miles</span>
            </div>
            <Btn onClick={addEstimateToLog} style={{ width:"100%", justifyContent:"center" }}>Add {estimateResult.total} miles to Mileage Log</Btn>
